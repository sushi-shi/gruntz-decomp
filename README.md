...
**Overall (vs full engine): 3,492 / 4,320 functions exact (80.83%) &middot; 94.95% fuzzy &middot; 94.97% fuzzy max.**

**Data — reconstructable coverage 99.76% &middot; gross coverage 56.47% &middot; fidelity 96.99%** (initialized data; `.bss` 92.02% / 96.65% is reported apart).

| Data region           |    retail B |  excluded B |  eligible B |  enrolled B |   coverage |   fidelity |
| :-------------------- | ----------: | ----------: | ----------: | ----------: | ---------: | ---------: |
| `.rdata`              |     135,168 |      65,283 |      69,885 |      69,685 |     99.71% |     96.41% |
| `.data` (initialized) |     136,192 |      52,477 |      83,715 |      83,547 |     99.80% |     97.57% |
| **initialized total** | **271,360** | **117,760** | **153,600** | **153,232** | **99.76%** | **96.99%** |
| `.bss` (zero fill)    |     627,628 |      18,433 |     609,195 |     560,551 |     92.02% |     96.65% |

_**Reconstructable coverage** = enrolled bytes divided by retail bytes not proven private and unreachable. **Gross coverage** retains all retail initialized bytes as its denominator. **Fidelity** = of enrolled bytes, the share objdiff scores byte-equal — the narrow question the old headline answered. The generated reachability partition proves 117,760 B private and unreachable, leaving 153,600 eligible B. Its 368 unenrolled B remain in the denominator, including every unclassified byte. Library-owned data directly reached from game/compiler code or through enrolled data is eligible, never excluded. `.bss` is partitioned the same way: 18,433 B excluded (zero-fill referenced only by proven library code, plus sub-alignment slack). `.bss` is kept out of the initialized total because zero-fill matching zero-fill is nearly free. objdiff's own `matched_data` is 330,992 / 804,985 B (41.12%, whole sections only): a PER-UNIT sum, so a COMDAT defined in N units counts N times — kept for history, never the headline. What the uncovered bytes ARE: `gruntz audit data_denominator`._
...
_Started units alone: 3,492/4,340 fns exact, 93.29% fuzzy over 1,134,143 of 1,114,375 engine code bytes._

| Module       | Units |       Functions exact |  Fuzzy | Fuzzy Max |
| :----------- | ----: | --------------------: | -----: | --------: |
| `Gruntz`     |   232 | 1,993 / 2,579 (77.3%) |  92.9% |     92.9% |
| `DDrawMgr`   |    22 |     499 / 584 (85.4%) |  92.4% |     92.4% |
| `Wwd`        |     9 |     166 / 216 (76.9%) |  93.7% |     93.7% |
| `Bute`       |     9 |     160 / 194 (82.5%) |  95.2% |     95.2% |
| `Dsndmgr`    |     4 |     147 / 156 (94.2%) |  97.8% |     97.8% |
| `Net`        |    12 |     117 / 134 (87.3%) |  97.7% |     97.7% |
| `Image`      |    10 |      86 / 121 (71.1%) |  93.7% |     93.7% |
| `Rez`        |     7 |       64 / 75 (85.3%) |  90.8% |     90.9% |
...
| `Utils`      |     7 |       21 / 26 (80.8%) |  98.8% |     98.8% |
| `Font`       |     1 |       17 / 21 (81.0%) |  86.9% |     86.9% |
| `Crypto`     |     2 |       13 / 17 (76.5%) |  88.9% |     88.9% |
...
| `private lifecycle/cleanup helpers` |       176 |    3,060 | volatile `$E<n>` families and anonymous cleanup forwards |
| `CRT/MFC library`                   |     1,981 |  199,316 | FID-identified, statically linked                        |
| `jump thunks`                       |     1,605 |    8,019 | linker ILT jmp-table (RVA <0x7950) + thunk_*             |
<!-- match-score:end -->
...
**Candidate image: 2,543,616 B vs retail 2,511,872 B (101.26%, +31,744 B).**

| section | retail | candidate | delta | retail bytes reproduced | not measurable |
| :-- | --: | --: | --: | --: | --: |
| `.data` | 763,820 | 837,548 | +73,728 | **10.17%** | 627,628 |
| `.idata` | 15,169 | 15,169 | +0 &check; | **78.03%** | 3,307 |
| `.rdata` | 135,080 | 141,936 | +6,856 | **17.87%** | 66,776 |
| `.reloc` | 113,105 | 113,525 | +420 | **42.88%** | 59,251 |
| `.rsrc` | 123,260 | 123,260 | +0 &check; | **100.00%** | 0 |
| `.text` | 1,987,179 | 2,010,513 | +23,334 | **50.11%** | 58,731 |
| **total** | 3,137,613 | 3,241,951 | +104,338 | **40.83%** | 815,693 |
...
_Independently of where the bytes landed, **36,464 of 36,746 address operands (99.23%) in paired regions reach the same referent in the same order**. That is the measure a byte diff cannot give and per-object objdiff masks away: it is what catches a wrong string literal or a call to the wrong function._
...
_Candidate `1659ddcc9af1`, linked 2026-08-10 19:18. Regenerate: `ninja candidate`._
<!-- link-score:end -->
