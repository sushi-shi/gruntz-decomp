...
**Overall (vs full engine): 3,407 / 4,314 functions exact (78.98%) &middot; 94.29% fuzzy &middot; 94.29% fuzzy max.**

**Data — reconstructable coverage 99.59% &middot; gross coverage 40.83% &middot; fidelity 100.00%** (initialized data; `.bss` 92.30% / 100.00% is reported apart).

| Data region           |    retail B |  excluded B |  eligible B |  enrolled B |   coverage |    fidelity |
| :-------------------- | ----------: | ----------: | ----------: | ----------: | ---------: | ----------: |
| `.rdata`              |     135,168 |     107,643 |      27,525 |      27,317 |     99.24% |     100.00% |
| `.data` (initialized) |     136,192 |      52,471 |      83,721 |      83,472 |     99.70% |     100.00% |
| **initialized total** | **271,360** | **160,114** | **111,246** | **110,789** | **99.59%** | **100.00%** |
| `.bss` (zero fill)    |     627,628 |      18,317 |     609,311 |     562,372 |     92.30% |     100.00% |

_**Reconstructable coverage** = enrolled bytes divided by retail bytes not proven private and unreachable. **Gross coverage** retains all retail initialized bytes as its denominator. **Fidelity** = of enrolled bytes, the share objdiff scores byte-equal — the narrow question the old headline answered. The generated reachability partition proves 160,114 B private and unreachable, leaving 111,246 eligible B. Its 457 unenrolled B remain in the denominator, including every unclassified byte. Library-owned data directly reached from game/compiler code or through enrolled data is eligible, never excluded. `.bss` is partitioned the same way: 18,317 B excluded (zero-fill referenced only by proven library code, plus sub-alignment slack). `.bss` is kept out of the initialized total because zero-fill matching zero-fill is nearly free. objdiff's own `matched_data` is 736,981 / 737,029 B (99.99%, whole sections only): a PER-UNIT sum, so a COMDAT defined in N units counts N times — kept for history, never the headline. What the uncovered bytes ARE: `gruntz audit data_denominator`._
...
_Started units alone: 3,407/4,327 fns exact, 92.65% fuzzy over 1,134,118 of 1,114,364 engine code bytes._

| Module       | Units |       Functions exact |  Fuzzy | Fuzzy Max |
| :----------- | ----: | --------------------: | -----: | --------: |
| `Gruntz`     |   232 | 1,945 / 2,574 (75.6%) |  92.2% |     92.2% |
| `DDrawMgr`   |    22 |     490 / 574 (85.4%) |  91.4% |     91.4% |
| `Wwd`        |     9 |     162 / 214 (75.7%) |  93.6% |     93.6% |
| `Bute`       |     9 |     145 / 191 (75.9%) |  94.2% |     94.2% |
| `Dsndmgr`    |     4 |     143 / 153 (93.5%) |  98.2% |     98.2% |
| `Net`        |    12 |     116 / 134 (86.6%) |  97.8% |     97.8% |
| `Image`      |    12 |      93 / 131 (71.0%) |  93.4% |     93.4% |
| `Rez`        |     7 |       62 / 76 (81.6%) |  91.0% |     91.0% |
| `zlib-1.0.4` |    11 |       61 / 63 (96.8%) | 100.0% |    100.0% |
| `DinMgr2`    |     2 |       55 / 59 (93.2%) | 100.0% |    100.0% |
| `Wap32`      |     7 |       53 / 57 (93.0%) |  99.7% |     99.7% |
| `Io`         |     4 |       34 / 38 (89.5%) |  99.0% |     99.0% |
| `Utils`      |     7 |       18 / 25 (72.0%) |  98.8% |     98.8% |
...
**Candidate image: 2,541,056 B vs retail 2,511,872 B (101.16%, +29,184 B).**

| section | retail | candidate | delta | retail bytes reproduced | not measurable |
| :-- | --: | --: | --: | --: | --: |
| `.data` | 763,820 | 837,420 | +73,600 | **10.16%** | 627,628 |
| `.idata` | 15,169 | 15,169 | +0 &check; | **77.57%** | 3,307 |
| `.rdata` | 135,080 | 142,119 | +7,039 | **17.87%** | 66,776 |
| `.reloc` | 113,105 | 113,422 | +317 | **42.78%** | 59,275 |
| `.rsrc` | 123,260 | 123,260 | +0 &check; | **100.00%** | 0 |
| `.text` | 1,987,179 | 2,007,880 | +20,701 | **49.83%** | 58,731 |
| **total** | 3,137,613 | 3,239,270 | +101,657 | **40.65%** | 815,717 |
...
_Independently of where the bytes landed, **36,202 of 36,534 address operands (99.09%) in paired regions reach the same referent in the same order**. That is the measure a byte diff cannot give and per-object objdiff masks away: it is what catches a wrong string literal or a call to the wrong function._
...
_Candidate `3fc09d607093`, linked 2026-08-10 13:26. Regenerate: `ninja candidate`._
<!-- link-score:end -->
