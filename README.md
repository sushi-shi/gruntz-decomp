...
**Overall (vs full engine): 3,498 / 4,312 functions exact (81.12%) &middot; 93.81% fuzzy &middot; 93.86% fuzzy max.**

<<<<<<< HEAD
**Data: 99.65% size-weighted across 704,132 B** (`.bss` 99.82%, `.data` 98.63%, `.rdata` 99.85%) &middot; 90.19% of bytes in sections that are exact, which is what objdiff's `matched_data` reports.
=======
**Data: 99.18% size-weighted across 704,132 B** (`.bss` 99.76%, `.data` 95.98%, `.rdata` 99.38%) &middot; 16.76% of bytes in sections that are exact, which is what objdiff's `matched_data` reports.
>>>>>>> 6ef39f5eb (netmgr: the five DirectPlay provider GUIDs live in retail's .data, not .rdata)
...
_Started units alone: 3,498/4,325 fns exact, 92.15% fuzzy over 1,113,083 of 1,093,329 engine code bytes._

| Module       | Units |       Functions exact |  Fuzzy | Fuzzy Max |
| :----------- | ----: | --------------------: | -----: | --------: |
| `Gruntz`     |   233 | 2,009 / 2,572 (78.1%) |  91.8% |     91.8% |
