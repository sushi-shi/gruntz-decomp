...
**Overall (vs full engine): 3,527 / 4,326 functions exact (81.53%) &middot; 95.68% fuzzy &middot; 95.68% fuzzy max.**

**Data — reconstructable coverage 99.77% &middot; gross coverage 56.47% &middot; fidelity 96.87%** (initialized data; `.bss` 92.23% / 96.70% is reported apart).

| Data region           |    retail B |  excluded B |  eligible B |  enrolled B |   coverage |   fidelity |
| :-------------------- | ----------: | ----------: | ----------: | ----------: | ---------: | ---------: |
| `.rdata`              |     135,168 |      65,283 |      69,885 |      69,685 |     99.71% |     96.21% |
| `.data` (initialized) |     136,192 |      52,482 |      83,710 |      83,562 |     99.82% |     97.51% |
| **initialized total** | **271,360** | **117,765** | **153,595** | **153,247** | **99.77%** | **96.87%** |
| `.bss` (zero fill)    |     627,628 |      18,441 |     609,187 |     561,863 |     92.23% |     96.70% |

_**Reconstructable coverage** = enrolled bytes ÷ eligible bytes (retail minus the proven-private `excluded B`). **Gross coverage** divides by all retail bytes; **fidelity** = of enrolled bytes, the share objdiff scores byte-equal. Unclassified bytes stay in the eligible denominator, and library-owned data reached from game/compiler code stays eligible. `.bss` is reported apart: zero-fill matching zero-fill is nearly free. objdiff's own `matched_data` (40.50%) is a per-unit sum (a shared COMDAT counts once per emitting unit) — history, never the headline. What the uncovered bytes ARE: `gruntz audit data_denominator`._
...
_Started units alone: 3,527/4,351 fns exact, 94.01% fuzzy over 1,134,668 of 1,114,867 engine code bytes._
