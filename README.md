...
**Overall (vs full engine): 3,504 / 4,314 functions exact (81.22%) &middot; 93.60% fuzzy &middot; 93.68% fuzzy max.**

**Data: 100.00% size-weighted across 723,491 B** (`.bss` 100.00%, `.data` 100.00%, `.rdata` 100.00%) &middot; 100.00% of bytes in sections that are exact, which is what objdiff's `matched_data` reports.
...
_Started units alone: 3,504/4,327 fns exact, 91.97% fuzzy over 1,134,118 of 1,114,364 engine code bytes._

| Module       | Units |       Functions exact |  Fuzzy | Fuzzy Max |
| :----------- | ----: | --------------------: | -----: | --------: |
| `Gruntz`     |   232 | 2,011 / 2,574 (78.1%) |  91.6% |     91.7% |
...
**Candidate image: 2,491,392 B vs retail 2,511,872 B (99.18%, -20,480 B).**

| section | retail | candidate | delta | retail bytes reproduced | not measurable |
| :-- | --: | --: | --: | --: | --: |
| `.data` | 763,820 | 832,524 | +68,704 | **10.06%** | 627,628 |
| `.idata` | 15,169 | 15,169 | +0 &check; | **78.03%** | 3,307 |
| `.rdata` | 135,080 | 142,728 | +7,648 | **16.96%** | 66,776 |
| `.reloc` | 113,105 | 110,236 | -2,869 | **41.05%** | 60,301 |
| `.rsrc` | 123,260 | 123,260 | +0 &check; | **100.00%** | 0 |
| `.text` | 1,987,179 | 1,961,366 | -25,813 | **48.55%** | 58,731 |
| **total** | 3,137,613 | 3,185,283 | +47,670 | **39.72%** | 816,743 |
...
_Independently of where the bytes landed, **35,572 of 36,670 address operands (97.01%) in paired regions reach the same referent in the same order**. That is the measure a byte diff cannot give and per-object objdiff masks away: it is what catches a wrong string literal or a call to the wrong function._

_`.rsrc` is byte-exact: all 140 differing bytes are `OffsetToData` fields shifted by the section placement delta, zero unexplained._
...
_Candidate `74c66f66b6fd`, linked 2026-08-09 12:07. Regenerate: `ninja candidate`._
<!-- link-score:end -->
