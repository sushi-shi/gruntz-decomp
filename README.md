...
**Overall (vs full engine): 3,498 / 4,312 functions exact (81.12%) &middot; 93.65% fuzzy &middot; 93.70% fuzzy max.**

**Data: 99.60% size-weighted across 704,132 B** (`.bss` 99.76%, `.data` 98.63%, `.rdata` 99.85%) &middot; 16.14% of bytes in sections that are exact, which is what objdiff's `matched_data` reports.
...
_Started units alone: 3,498/4,325 fns exact, 91.99% fuzzy over 1,113,083 of 1,093,329 engine code bytes._

| Module       | Units |       Functions exact |  Fuzzy | Fuzzy Max |
| :----------- | ----: | --------------------: | -----: | --------: |
| `Gruntz`     |   233 | 2,009 / 2,572 (78.1%) |  91.6% |     91.6% |
