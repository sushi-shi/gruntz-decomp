...
**Overall (vs full engine): 3,538 / 4,326 functions exact (81.78%) &middot; 95.76% fuzzy &middot; 95.76% fuzzy max.**

**Data — reconstructable coverage 100.00% &middot; gross coverage 56.56% &middot; fidelity 96.77%** (initialized data; `.bss` 92.23% / 96.70% is reported apart).

| Data region           |    retail B |  excluded B |  eligible B |  enrolled B |    coverage |   fidelity |
| :-------------------- | ----------: | ----------: | ----------: | ----------: | ----------: | ---------: |
| `.rdata`              |     135,168 |      65,314 |      69,854 |      69,854 |     100.00% |     95.99% |
| `.data` (initialized) |     136,192 |      52,573 |      83,619 |      83,619 |     100.00% |     97.53% |
| **initialized total** | **271,360** | **117,887** | **153,473** | **153,473** | **100.00%** | **96.77%** |
| `.bss` (zero fill)    |     627,628 |      18,441 |     609,187 |     561,863 |      92.23% |     96.70% |

_**Reconstructable coverage** = enrolled bytes ÷ eligible bytes (retail minus the proven-private `excluded B`). **Gross coverage** divides by all retail bytes; **fidelity** = of enrolled bytes, the share objdiff scores byte-equal. Unclassified bytes stay in the eligible denominator, and library-owned data reached from game/compiler code stays eligible. `.bss` is reported apart: zero-fill matching zero-fill is nearly free. objdiff's own `matched_data` (44.20%) is a per-unit sum (a shared COMDAT counts once per emitting unit) — history, never the headline. What the uncovered bytes ARE: `gruntz audit data_denominator`._
...
_Started units alone: 3,538/4,350 fns exact, 94.09% fuzzy over 1,134,641 of 1,114,867 engine code bytes._

| Module        | Units |       Functions exact |  Fuzzy | Fuzzy Max |
| :------------ | ----: | --------------------: | -----: | --------: |
| `Gruntz`      |   213 | 2,019 / 2,583 (78.2%) |  93.7% |     93.7% |
| `DDrawMgr`    |    21 |     507 / 586 (86.5%) |  92.7% |     92.7% |
| `Wwd`         |     9 |     166 / 217 (76.5%) |  95.0% |     95.0% |
| `Bute`        |     7 |     165 / 196 (84.2%) |  97.1% |     97.1% |
| `Dsndmgr`     |     4 |     148 / 156 (94.9%) |  99.5% |     99.5% |
| `Net`         |     9 |     121 / 135 (89.6%) |  97.4% |     97.4% |
| `Image`       |     8 |      82 / 118 (69.5%) |  94.8% |     94.8% |
