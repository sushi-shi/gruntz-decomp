...
**Overall (vs full engine): 2,946 / 4,314 functions exact (68.29%) &middot; 94.07% fuzzy &middot; 94.07% fuzzy max.**

**Data — reconstructable coverage 99.67% &middot; gross coverage 40.96% &middot; fidelity 100.00%** (initialized data; `.bss` 92.33% / 100.00% is reported apart).

| Data region           |    retail B |  excluded B |  eligible B |  enrolled B |   coverage |    fidelity |
| :-------------------- | ----------: | ----------: | ----------: | ----------: | ---------: | ----------: |
| `.rdata`              |     135,168 |     107,379 |      27,789 |      27,589 |     99.28% |     100.00% |
| `.data` (initialized) |     136,192 |      52,479 |      83,713 |      83,549 |     99.80% |     100.00% |
| **initialized total** | **271,360** | **159,858** | **111,502** | **111,138** | **99.67%** | **100.00%** |
| `.bss` (zero fill)    |     627,628 |      18,333 |     609,295 |     562,540 |     92.33% |     100.00% |

_**Reconstructable coverage** = enrolled bytes divided by retail bytes not proven private and unreachable. **Gross coverage** retains all retail initialized bytes as its denominator. **Fidelity** = of enrolled bytes, the share objdiff scores byte-equal — the narrow question the old headline answered. The generated reachability partition proves 159,858 B private and unreachable, leaving 111,502 eligible B. Its 364 unenrolled B remain in the denominator, including every unclassified byte. Library-owned data directly reached from game/compiler code or through enrolled data is eligible, never excluded. `.bss` is partitioned the same way: 18,333 B excluded (zero-fill referenced only by proven library code, plus sub-alignment slack). `.bss` is kept out of the initialized total because zero-fill matching zero-fill is nearly free. objdiff's own `matched_data` is 737,254 / 737,302 B (99.99%, whole sections only): a PER-UNIT sum, so a COMDAT defined in N units counts N times — kept for history, never the headline. What the uncovered bytes ARE: `gruntz audit data_denominator`._
...
_Started units alone: 2,946/4,327 fns exact, 92.43% fuzzy over 1,134,118 of 1,114,364 engine code bytes._

| Module       | Units |       Functions exact |  Fuzzy | Fuzzy Max |
| :----------- | ----: | --------------------: | -----: | --------: |
| `Gruntz`     |   232 | 1,654 / 2,574 (64.3%) |  92.2% |     92.2% |
| `DDrawMgr`   |    22 |     392 / 574 (68.3%) |  89.5% |     89.5% |
| `Wwd`        |     9 |     150 / 214 (70.1%) |  93.6% |     93.6% |
| `Bute`       |     9 |     135 / 191 (70.7%) |  94.2% |     94.2% |
| `Dsndmgr`    |     4 |     131 / 153 (85.6%) |  98.2% |     98.2% |
| `Net`        |    12 |     111 / 134 (82.8%) |  97.7% |     97.7% |
| `Image`      |    12 |      75 / 131 (57.3%) |  93.4% |     93.4% |
| `Rez`        |     7 |       57 / 76 (75.0%) |  90.8% |     90.8% |
| `zlib-1.0.4` |    11 |      63 / 63 (100.0%) | 100.0% |    100.0% |
| `DinMgr2`    |     2 |       52 / 59 (88.1%) | 100.0% |    100.0% |
| `Wap32`      |     7 |       52 / 57 (91.2%) |  99.7% |     99.7% |
| `Io`         |     4 |       31 / 38 (81.6%) |  98.9% |     98.9% |
