// CSeverusEntryList.h - an owned-collection node of the DDrawMgr "severus worker"
// family (placeholder name; engine "ClassUnknown_35"). Non-RTTI engine class; its
// own primary vtable is at RVA 0x1efbe8 (g_severusEntryListVtbl). It derives from a
// CSeverusWorker-shaped base subobject (m_04/m_08/m_0c reset on teardown, then the
// CObject-like grand-base dtor vtable g_severusBaseDtorVtbl @0x5e8cb4 restored).
//
// Layout recovered from the dtor (0x1557a0) + DeleteAll helper (0x151eb0):
//   +0x00 vptr (CSeverusBase)
...
#ifndef GRUNTZ_CSEVERUSENTRYLIST_H
#define GRUNTZ_CSEVERUSENTRYLIST_H
...
struct SeverusObject {
    struct Vtbl {
        void* m_slot0;
        void* (SeverusObject::*m_deleteDtor)(u32); // +0x04  scalar-deleting dtor
    }* m_vptr;
};
...
struct SeverusObArray {
    void* m_vptr;            // +0x00
    SeverusObject** m_pData; // +0x04
    i32 m_nSize;             // +0x08
    i32 m_nMaxSize;          // +0x0c
    i32 m_nGrowBy;           // +0x10
...
// The grand-base (CObject-like) dtor vtable restored at ~CSeverusBase exit.
extern void* g_severusBaseDtorVtbl; // 0x5e8cb4

// The "severus worker" base subobject: vptr @ +0x00 + three managed fields. Its
