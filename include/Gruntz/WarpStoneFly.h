...
// UpdateWarpStoneStatusBar/m_54c teardown drive it. No vtable (m_mode is a mode int,
// not a vptr), so no RTTI for this leaf class.
//
// The overlay interpolates a sprite (m_sprite) from a current float position
// (m_currentX/m_currentY) toward an integer target (m_targetX/m_targetY) along a per-frame velocity
// (m_speed/m_velocityX/m_velocityY), snapping to the target on overshoot. On arrival it pokes the
// owner's mode byte into the registry tab array and frees itself off the owner.
//
// Offsets + code bytes are the load-bearing fact; the
...
struct CWsfOwner {
    ...
    i32 m_activeTab; // +0x10c  active-tab id (==5 arms the tab switch)
    char m_pad110[0x548 - 0x110];
    i32 m_busy;      // +0x548  busy flag, cleared on arrival
    void* m_overlay; // +0x54c  this overlay (freed + nulled on arrival)
};
...
public:
    ...
    i32 m_mode;    // +0x00  mode value poked into the registry tab array on arrival
    ...
    double m_speed;       // +0x20  velocity term a
    double m_velocityX;   // +0x28  velocity term b (x sign gate)
    double m_velocityY;   // +0x30  velocity term c (y sign gate)
