...
//   0x17c6f0  CMoviePlayer::Open   (src/Gruntz/CMoviePlayer.cpp, frameless)
//   0x038fc0  CMoviePlayer::~CMoviePlayer (src/Gruntz/CMoviePlayerEh.cpp, /GX)
...
// The Rez-owned scratch embed at worker+0x868c: a CLoadable-shaped subobject
// (vptr + one owned buffer). Its teardown stamps its own vtable, RezFree's the
// buffer, then restores the shared base object dtor vtable.
struct CMovieScratch {
...
public:
    ...
    CMovieDecodeStore m_540; // +0x540
    char m_pad740[0x868c - (0x540 + 0x200)];
    CMovieScratch m_868c; // +0x868c
};
