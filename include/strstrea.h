...
public:
    ...
    // The era header declares NO ~strstream: GRUNTZ.EXE contains no
    // ??1strstream/??_Dstrstream anywhere, and CButeMgr::Save's iostream&-cast
    // strstream temp tears down via ??1iostream+??1ios (normal path AND the
    // state-1 funclet -> ??_Diostream). With the toolchain's reconstructed
    // STRSTREA.H (which guesses a dtor - see its "strstreambase ???" comment)
    // cl is forced to reference/synthesize the strstream dtor in every
    // spelling. This corrected copy shadows it; buffer cleanup is ~ios/delbuf.

    inline int pcount() const {
        return rdbuf()->out_waiting();
    } // not in spec.
    inline strstreambuf* rdbuf() const {
        return (strstreambuf*)ostream::rdbuf();
    }
    inline char* str() {
        return rdbuf()->str();
    }
