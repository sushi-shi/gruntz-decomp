...
//
//   * NetThingDtor.cpp    `struct CNetThing : public CObList { ~CNetThing(); }`
//     CObList (MFC) has a virtual destructor, so CNetThing's is implicitly virtual too
...
// runs CKeyedList::Clear then chains the ~CObList base - so CNetThing and CKeyedList are
...
#include <Mfc.h> // real MFC CObList (the base whose virtual dtor 0x1b48c6 the chain hits)
#include <rva.h>

SIZE_UNKNOWN(CNetThing); // dtor-only shape; retail size TBD
struct CNetThing : public CObList {
    ~CNetThing(); // 0xc5280 - implicitly VIRTUAL (CObList's dtor is); ??1CNetThing@@UAE@XZ
};
