#!/usr/bin/env python3
"""gruntz.core.vtable_hierarchy - per-class virtual table with every slot
...
Usage:
    python3 -m gruntz.core.vtable_hierarchy                 # full per-class slot tables
    python3 -m gruntz.core.vtable_hierarchy --summary       # one line per class
    python3 -m gruntz.core.vtable_hierarchy --class CImage  # one class, full detail
    python3 -m gruntz.core.vtable_hierarchy --csv out.csv   # machine-readable, all classes
    python3 -m gruntz.core.vtable_hierarchy --coverage      # completeness: anchored vs UNANCHORED (omitted) src vtables
    python3 -m gruntz.core.vtable_hierarchy --name-audit    # src-name vs RTTI-COL-name mismatches (RTTI authoritative)
"""
import argparse
import csv
import re
...
class Audit:
    ...
    def vtable_bearing(self):
        ...
        game-catalog row, or an RTTI ??_7 in config/retail/data_vtables.tsv."""
...
def cmd_coverage(aud):
    ...
    print("# named in config/retail/data_vtables.tsv), and whether the analyzer can ANCHOR")
...
def cmd_audit(aud):
    ...
    # MFC/CRT library classes (config/retail/data_static_libs.tsv) are statically linked, NOT
