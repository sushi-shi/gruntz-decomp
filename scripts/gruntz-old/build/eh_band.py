...
THE STUB'S OPERAND IS A DATUM, AND IT NEEDS THE SAME TREATMENT.  `mov eax,<FuncInfo>`
addresses the `.xdata$x` blob cl emits beside the funclets, which is anonymous on
both sides for the same reason the funclets were: cl labels it `$T<n>` and retail's
image has no symbol at all.  So the same owner-derived scheme names it -

    __ehfuncinfo$<owner>     the 32-byte `_s_FuncInfo` record the stub loads
    __ehunwindmap$<owner>    the `8 * maxState` unwind map that immediately follows

and both extents are PROVEN out of the record rather than assumed: the blob enrolls
only when its own `pUnwindMap` word points at `funcinfo + 32` (which is what fixes
the record's length) and its try-block / ip-to-state maps are empty (which is what
makes the unwind map the whole tail).  All 750 of GRUNTZ.EXE's groups satisfy both.
"""
...
#: cl 5.0's `_s_FuncInfo` record, as the shipped image states it: the unwind map of
#: every one of the 750 groups begins exactly here, which is the record's length.
FUNCINFO_SIZE = 32
...
def is_band_symbol(name: str) -> bool:
    """True for the two CODE names this module carves (used by the metric filters)."""
...
def _funclets(image: _Image, funcinfo_rva: int) -> tuple[set[int], int, bool, bool] | None:
    ...
    if magic != FUNCINFO_MAGIC or states < 0 or tries < 0:
        return None
    # The record itself states where its unwind map is; when that is the word right
    # after the record, the record's LENGTH is proven and the map is its whole tail.
...
def data_records(band: list[Group]) -> list[tuple[int, str, str, int]]:
    ...
    proves (see `Group.funcinfo_size`); a group whose record does not prove one is
    silently absent rather than guessed at.  These are the rows synth_pdb names in
    the PDB and `gruntz.build.data_manifest` enrolls, which together are what turns
    the stub's `mov eax,<FuncInfo>` from a reference to an unenrolled blob into a
    reference to a named, sized, relocated datum on both sides.
    """
    out = []
    for group in band:
