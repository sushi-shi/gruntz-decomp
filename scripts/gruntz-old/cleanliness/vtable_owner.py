...
catalog bindings turned out to point at the WRONG class (a silent wrong-dispatch bug that a
green gate cannot see, because uniqueness and coverage are both satisfied by a lie).
    ...
    python -m gruntz.cleanliness.vtable_owner --audit        # re-derive every game binding
...
def src_claims():
    """({class: (rva, catalog:line)}, {class: (dtor_rva, file:line)})."""
    vtbl = {}
    for row in vtable_catalog.game_rows():
        name = vtable_catalog.primary_class(row["name"])
        if name:
            where = f"{row['path'].relative_to(REPO)}:{row['line']}"
            vtbl[name] = (row["rva"], where)
    dtor = {}
    for p in _src_files():
...
def explain(rva, vtbl_by_class):
    ...
    for v, k, via, sdd in hits:
        ...
        if claim:
            print("      catalog binds this rva: %s  [%s]" % (claim[0], claim[1]))
        else:
            print("      catalog binds this rva: (nothing)")
...
def rtti_leaf(decorated):
    ...
    identifier, so a simple primary catalog name is wrong by construction)."""
    s = decorated
...
def rtti_check(vtbl):
    """RTTI cross-check: for every primary catalog claim carrying an RTTI Complete
    Object Locator, the COL's class name is GROUND TRUTH. A claim whose
    ...
    ??_7cls@@6B@ primary name) - is a MISBINDING the dtor-based audit cannot see
    ...
    for cls, (rva, where) in sorted(vtbl.items(), key=lambda kv: kv[1][0]):
        ...
        print("      catalog: 0x%06x   %s" % (rva, where))
        print("      rtti: %s" % why)
    return bad


def audit(show_all=False):
    """Re-derive every primary game-catalog binding from the binary.
    ...
    MISBINDING - a wrong-dispatch bug that catalog uniqueness + vtable-coverage both pass,
    ...
    name must agree with the catalog class name (see rtti_check)."""
    ...
    for cls, (drva, dwhere) in sorted(dtor.items()):
        ...
        if claimed in starts:
            agree += 1
            if show_all:
                print("  OK %-28s catalog 0x%06x <- dtor 0x%06x" % (cls, claimed, drva))
            continue
        bad += 1
        print("  MISBOUND %s" % cls)
        print("      catalog: 0x%06x   %s" % (claimed, where))
...
def main():
    ...
    ap.add_argument("--audit", action="store_true", help="re-derive every primary game-catalog binding")
