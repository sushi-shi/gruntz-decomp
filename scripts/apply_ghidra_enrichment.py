...
ROOT = "/home/sheep/Projects/gruntz"
CSV_ENGINE = ROOT + "/config/engine_labels.csv"
CSV_SYMBOL = ROOT + "/config/symbol_names.csv"
...
try:
    ...
    n_structs = 0
    struct_dt_by_name = {}
    for (name, size, apply_this, desc, fields) in STRUCTS:
    ...
    # ---- (B) define enums ----
    n_enums = 0
    for (name, size, desc, members) in ENUMS:
        define_enum(name, size, desc, members)
        n_enums += 1
    R("enums defined: %d" % n_enums)
