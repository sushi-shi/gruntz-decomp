#!/usr/bin/env python3
"""gruntz.core.inline_model - the cl 5.0 per-caller inline-budget model.
...
USAGE
    python -m gruntz.core.inline_model --selftest
    python -m gruntz.core.inline_model --spec sites.json [--json]
...
_MODEL_SCRATCH = REPO / "build/inline-model"
...
def measure_cb(src: Path, callee: str, caller: str, n_sites: int):
    ...
    cmd = [sys.executable, "-m", "gruntz.core.cc_wrap",
           "--out", str(out), "--src", str(src), "--", "/c", *_PROFILE]
    proc = subprocess.run(cmd, capture_output=True, text=True)
    asm = _MODEL_SCRATCH / (src.stem + ".asm")
    if not asm.is_file():
        # cl drops the /FAs listing beside the OBJECT it writes
        asm = out.with_suffix(".asm")
    if proc.returncode != 0 or not asm.is_file():
        die("measure-cb compile failed:\n" +
...
def main() -> int:
    ...
    ap = argparse.ArgumentParser(
        prog="python -m gruntz.core.inline_model",
