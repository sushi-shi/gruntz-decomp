"""gruntz.core.paths - repo and toolchain path discovery, once.
    ...
    for base in (Path.cwd(), Path(__file__).resolve().parent):
        for p in (base, *base.parents):
            if (p / "flake.nix").exists():
                return p
    raise RuntimeError("not inside a gruntz repo (no flake.nix upward of cwd)")
...
def retail_exe() -> Path:
    return Path(os.environ.get("GRUNTZ_EXE") or BUILD / "exe/GRUNTZ.EXE")
