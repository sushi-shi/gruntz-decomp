...
from gruntz.core.tsv import read, rint
...
def inventory_main(args) -> int:
    ...
    print(f"{'wave':>4}  {'hist':>6}  {'decision':<13}  {'module':<10}  source -> Gruntz")
    for row in rows[: args.limit]:
        hist = f"{row['hist']:6.2f}" if row["hist"] is not None else "     -"
        target = row["gruntz_symbol"] or "-"
        print(
...
def verify_main(args) -> int:
    ...
    if args.source:
        ...
        from gruntz.lineage.discovery import discover
