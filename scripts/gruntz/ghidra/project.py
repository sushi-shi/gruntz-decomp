"""gruntz.ghidra.project - build, refresh and read back the viewer database.

    gruntz ghidra build  [--force] [--aggressive] [--no-bookmarks]
    gruntz ghidra update [--force] [--no-bookmarks]
    gruntz ghidra verify [rva ...]
    gruntz ghidra status

`build` imports the retail image into build/ghidra/gruntz.gpr, auto-analyzes it
...
from gruntz.core.paths import BUILD, retail_exe
from gruntz.ghidra import export
from gruntz.tool import ToolError, ghidra
...
def _apply_env(bookmarks: bool) -> dict[str, str]:
    return {"GRUNTZ_GHIDRA_PAYLOAD": str(export.PAYLOAD),
            "GRUNTZ_GHIDRA_BOOKMARKS": "1" if bookmarks else "0"}
...
def status() -> int:
    ...
    if not exists():
        print("[ghidra] state       : run `gruntz ghidra build`")
    elif stamp.get("digest") == doc["digest"]:
        print("[ghidra] state       : up to date")
    else:
        print("[ghidra] state       : STALE - run `gruntz ghidra update`")
    _report(doc)
    return 0
