# apply.py - push the Gruntz reconstruction's knowledge into the viewer DB.
#
...
# tree: its single input is the payload gruntz.ghidra.export wrote, whose path
# arrives in $GRUNTZ_GHIDRA_PAYLOAD. Nothing here re-derives a fact.
#
...
BEGIN = "[gruntz]"
END = "[/gruntz]"
BAND_BEGIN = "[gruntz-band]"
BAND_END = "[/gruntz-band]"
BAND_CATEGORY = "gruntz-band"
...
PAYLOAD = os.environ["GRUNTZ_GHIDRA_PAYLOAD"]
BOOKMARKS = os.environ.get("GRUNTZ_GHIDRA_BOOKMARKS", "1") != "0"
...
# ===================================================================== RUN
R("=== gruntz ghidra export: applying %s ===" % PAYLOAD)
R("payload schema=%d digest=%s image_base=0x%x"
  % (DOC["schema"], DOC["digest"][:12], BASE))

tx = prog.startTransaction("gruntz-knowledge-export")
committed = False
try:
    ...
    tree = None
    try:
        tree = listing.getRootModule("gruntz-bands")
        if tree is None:
            tree = listing.createRootModule("gruntz-bands")
    except Exception:
        tree = None
    n_band = n_frag = 0
    for band in band_rows:
        ...
        if tree is not None:
            try:
                ...
                frag = listing.getFragment("gruntz-bands", name)
                if frag is None:
