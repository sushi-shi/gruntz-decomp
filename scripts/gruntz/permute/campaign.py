...
from gruntz.permute.generate_ast_variants import main as variants_main
from gruntz.permute.tu_state_noise import load_units, project_root
from gruntz.verify.scores import is_eh_band
from gruntz.walls import diagnose as wall_diagnose
    ...
    for index, row in enumerate(selected, 1):
        ...
        try:
            exit_code = variants_main(
                command, prog="gruntz permute campaign", description=__doc__
            )
...
def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(
        prog="gruntz permute", description=__doc__,
