"""gruntz.tool.rez - the Monolith RezMgr v1 archive (Gruntz.REZ), read-only.
...
given frame or silently stores m_frame = NULL.
    ...
    gruntz tool rez list <archive> [--path SUB] [--index]
    gruntz tool rez extract <archive> <path> <out>
"""
...
def do_extract(args) -> int:
    ...
    print(f"[rez] no resource named {args.path!r} "
          f"(`gruntz tool rez list` shows the tree)", file=sys.stderr)
    return 1


def main(argv: list[str] | None = None) -> int:
    ap = argparse.ArgumentParser(prog="gruntz tool rez",
