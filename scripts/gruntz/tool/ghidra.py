"""gruntz.tool.ghidra - headless Ghidra, for the one-way viewer export.

    gruntz tool ghidra --exe E --project-dir D --project N --script S [...]

In-process (same function):
    from gruntz.tool import ghidra
    out = ghidra.headless(exe, project_dir, "gruntz", [apply_py],
                          analyze=False, env={"GRUNTZ_GHIDRA_PAYLOAD": p})
...
PyGhidra bootstrap `gruntz/ghidra/headless.py`, executed by THIS interpreter in
...
means is gruntz.ghidra's business. Ghidra is optional: no GHIDRA_INSTALL_DIR
...
#: an import - the tool layer stays free of gruntz.ghidra's policy)
...
def main(argv: list[str] | None = None) -> int:
    import argparse
    ap = argparse.ArgumentParser(prog="gruntz tool ghidra",
                                 description=__doc__.splitlines()[0])
    ap.add_argument("--exe", required=True)
    ap.add_argument("--project-dir", required=True)
    ap.add_argument("--project", default="gruntz")
