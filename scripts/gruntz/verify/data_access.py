"""gruntz.verify.data_access - the RETAIL-side data-access audit (full tier).
...
Engine + schema: gruntz.verify.access_map (retail bytes and .reloc from
...
carry the measured rate and the reason, one line each.
    ...
    python3 -m gruntz.verify.data_access --build | --gate | --calibrate
    python3 -m gruntz.verify.data_access --selftest      inject 9 known
                                                         defects, require each
    python3 -m gruntz.verify.data_access --suppressed [class]   the sites a
                                                         suppression removed
    python3 -m gruntz.verify.data_access --findings [category] [--limit N]
    python3 -m gruntz.verify.data_access --at 0x24e360 | --symbol NAME
    python3 -m gruntz.verify.data_access --fn NAME | --sql "SELECT ..."
    python3 -m gruntz.verify.data_access --touched       the byte ranges retail
...
from gruntz.verify.access_map import (SQLITE, STRING_OPS, TSV, Claim, connect,
                                      load, persist)
...
def analyse(trace=None):
    ...
    ride a stale map."""
    from gruntz.verify.access_map import Owners
...
def do_touched(args):
    ...
    write_tsv(args.touched,
              ["# GENERATED by gruntz.verify.data_access --touched - the byte "
...
def main(argv=None) -> int:
    ...
    ap = argparse.ArgumentParser(prog="gruntz verify data-access",
                                 description=__doc__,
