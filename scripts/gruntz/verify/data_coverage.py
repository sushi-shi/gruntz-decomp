"""gruntz.verify.data_coverage - is every datum MODELLED, not merely byte-equal?
...
range (`touched`/`sites`, from gruntz.verify.data_access's sweep).
...
def load_claims(path=MANIFEST):
    ...
    if not path.is_file():
        raise SystemExit(f"no {path} - run `gruntz build` first")
...
def main(argv=None) -> int:
    import argparse
    ap = argparse.ArgumentParser(prog="gruntz verify data-coverage",
                                 description=__doc__,
    ...
    if a.tsv:
        changed = write_tsv(
            a.tsv, ["# GENERATED by gruntz.verify.data_coverage - retail bytes "
