"""gruntz.verify.assert_relocs - the reloc-TARGET multiset audit (full tier).
...
externals and own-obj definitions always link and are never FAKE.
    ...
    python3 -m gruntz.verify.assert_relocs [--unit U] [0xRVA]
"""
...
from gruntz.walls.pairscan import DIR32, REL32, canon
...
def audit(unit_filter=None, review_rva=None):
    from gruntz.verify.undefined_closure import lib_symbols
    ...
    for unit in sorted(near):
        ...
        for name in sorted(near[unit]):
            ...
            # DIR32: every .reloc HIGHLOW fixup inside the extent (this also
...
def main(argv=None) -> int:
    import argparse
    ap = argparse.ArgumentParser(prog="gruntz verify assert-relocs",
                                 description=__doc__,
