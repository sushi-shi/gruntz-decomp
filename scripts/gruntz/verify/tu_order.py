"""gruntz.verify.tu_order - the linker-layout acceptance gate (normal tier).
...
writes the baseline (bless with --update).
    ...
    python3 -m gruntz.verify.tu_order [--gate] [--tu NAME] [--update]
"""
...
from gruntz.core.paths import BUILD, CONFIG, REPO, SRC
from gruntz.core.tsv import read as read_tsv
from gruntz.verify.srcscan import RVA_RE
...
def main(argv=None) -> int:
    ...
    if findings:
        print("fix the layout, never bless it up "
              "(`python3 -m gruntz.verify.tu_order --tu <name>` for detail)",
              file=sys.stderr)
