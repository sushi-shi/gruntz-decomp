"""gruntz.verify.undefined_closure - fake views + declared-only aliases (normal).
...
names. Two reports off one closure:
  ...
  * DECLARED-ONLY debt (declared_only): the rest - free functions, methods on
    ...
    python3 -m gruntz.verify.undefined_closure [--list] [--update]
"""
...
from gruntz.core.paths import BUILD, CONFIG, REPO
...
def _write_baseline(syms) -> None:
    rows = "\n".join(sorted(syms))
    BASELINE.write_text(
        "# declared-only debt (gruntz.verify.undefined_closure): symbols some "
...
def gate_findings() -> list[str]:
    if not BASE.is_dir() or not any(BASE.glob("*.obj")):
        return ["undefined-closure: no base objs - run `gruntz build` first "
                "(never vacuous)"]
