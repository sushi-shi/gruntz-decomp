"""gruntz.verify.vtable_scan - every vtable in GRUNTZ.EXE, with exact sizes.

Three independent retail signals (ported; rebuilt on gruntz.sema.image):
  1. a maximal stride-4 run of DATA-section .reloc sites whose stored values
...
head with no signal: EH/jump tables, NOT a vtable).
    ...
    python3 -m gruntz.verify.vtable_scan [--new] [--holds 0xRVA] [--dump 0xRVA]
"""
...
def main(argv=None) -> int:
    import argparse
    ap = argparse.ArgumentParser(prog="gruntz verify vtable-scan",
                                 description=__doc__)
