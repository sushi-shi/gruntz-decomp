"""gruntz.verify.verbs - status / check / bank.
...
from gruntz.verify import readme as rm
from gruntz.verify import scores
from gruntz.verify.baseline import EPS
from gruntz.verify.fingerprints import fingerprinter, is_fallback, real_edit
...
def _reconcile(overall, mods, started_fzw, started_code, eng, cur,
    ...
    print("the numbers below are recomputed by gruntz.verify. The new numbers")
...
def cmd_bank(argv) -> int:
    ...
    if not a.no_refresh:
        from gruntz.verify.fingerprints import regenerate
        regenerate()
