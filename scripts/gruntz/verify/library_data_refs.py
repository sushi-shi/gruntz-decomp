...
reported by ``gruntz verify data-coverage --tsv``.

    gruntz verify library-data-refs
    gruntz verify library-data-refs --near 0x1eafbc
    gruntz verify library-data-refs --tsv
"""
...
from gruntz.compare.canonicalize import CoffObject, DIR32, Symbol
from gruntz.core.paths import BUILD, RETAIL, msvc_dir
from gruntz.core.tsv import read as read_tsv
from gruntz.delink.implib import _ar_members
...
def main(argv=None) -> int:
    import argparse
    ap = argparse.ArgumentParser(prog="gruntz verify library-data-refs",
                                 description=__doc__,
    ...
    for required in (GAPS_TSV, ACCESS_TSV):
        if not required.is_file():
            ap.error(f"missing {required}; run gruntz verify data-coverage --tsv")
