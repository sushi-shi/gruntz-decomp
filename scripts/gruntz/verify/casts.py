"""gruntz.verify.casts - the cast campaign's one check (fast tier).
...
Three ported behaviors, one module (they are one campaign):
  ...
  * NESTED static_casts (merged from nested_static_casts.py, full tier): a
    ...
    python3 -m gruntz.verify.casts            # summary + the OPEN worklist
    python3 -m gruntz.verify.casts --gate     # ledger + self-recursion gate
    python3 -m gruntz.verify.casts --nested   # the AST scan (needs compdb)
"""
