"""gruntz.verify.access_map - THE static map of every data access retail makes.
...
PLUMBING (the port): retail bytes + relocations from gruntz.sema.image, the
decode from gruntz.tool.objdump, the claim spine from gruntz.model (which
replaced symbol_names.csv), and the declared field layout from
gruntz.verify.layout (pylibclang under the TU's own i386/MSVC flags).
"""
...
def persist(img, layout, accesses, cells, claims, stats, sqlite_path=SQLITE,
    ...
    write_tsv(tsv_path, ["# GENERATED by gruntz.verify.access_map - every "
...
def connect(path=SQLITE):
    if not Path(path).exists():
        raise SystemExit(f"no access map at {path} - run "
                         f"`python3 -m gruntz.verify.data_access --build`")
...
def load():
    ...
    from gruntz.verify.layout import harvest
    img = retail()
    model = resolve()
