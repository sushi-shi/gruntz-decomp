"""gruntz.verify.alloc_size - the retail ALLOCATION-SIZE oracle (full tier).
...
(cached in build/gen/class_sizes.json, keyed on a src/include tree hash).
    ...
    python3 -m gruntz.verify.alloc_size [--all] [--sites] [--class C]
"""
...
def rtti_ancestors() -> dict[str, set[str]]:
    ...
    if _RTTI_ANC is not None:
        return _RTTI_ANC
    from gruntz.sema.image import retail
    from gruntz.verify import vtable_scan as vs
    img = retail()
    base = img.base
...
def def_counts() -> dict[str, int]:
    from gruntz.verify.srcscan import iter_class_defs
