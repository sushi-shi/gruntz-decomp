"""gruntz.verify.board - the cleanliness scoreboard (fast tier).
...
never blessed away (the silently-dropped-floor lesson).
    ...
    python3 -m gruntz.verify.board            # counts + delta vs the floors
    python3 -m gruntz.verify.board --gate     # exit 1 on any ratchet rise
    python3 -m gruntz.verify.board --semantic # include the build-derived rows
    python3 -m gruntz.verify.board --update   # bless: write the floor files
"""
...
from gruntz.core.paths import CONFIG, REPO
...
def _count_unexplained_casts(code: str) -> int:
    ...
    from gruntz.verify.casts import CAST, FORCED, REASON
    lines = code.split("\n")
    n = 0
...
#: the four manual-vtable idioms - the drove-to-0 class gruntz.verify.bans
...
def semantic_count() -> list[tuple[str, int]]:
    ...
    values: dict[str, int] = {}
    try:
        from gruntz.verify.casts import nested_static_casts
        ...
        print(f"  board: nested static_casts UNMEASURED "
              ...
              file=sys.stderr)
    try:
        from gruntz.verify.caller_callee import fake_view_count
