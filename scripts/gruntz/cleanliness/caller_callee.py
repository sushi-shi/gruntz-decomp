#!/usr/bin/env python3
"""gruntz.cleanliness.caller_callee - retail<->reconstruction CALL-GRAPH reconciliation metric.
...
Usage (inside `nix develop` - needs clang + $GRUNTZ_EXE + build/clangd + symbol_names):
    python3 -m gruntz.cleanliness.caller_callee              # summary + metric + top causes
    python3 -m gruntz.cleanliness.caller_callee --metric     # just the numbers (tracking)
    python3 -m gruntz.cleanliness.caller_callee --worklist    # FAKE-VIEW dissolution worklist
    python3 -m gruntz.cleanliness.caller_callee --check 0xb5460   # one caller: retail vs emitted
    python3 -m gruntz.cleanliness.caller_callee --csv         # unreconciled edges, machine-readable
    python3 -m gruntz.cleanliness.caller_callee --md          # -> config/caller-callee-reconcile.md
    python3 -m gruntz.cleanliness.caller_callee --jobs 24
"""
import argparse
import bisect
