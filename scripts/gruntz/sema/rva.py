"""gruntz.sema.rva - the one-shot address dossier.

    python3 -m gruntz.sema.rva 0x2bf228
    python3 -m gruntz.sema.rva 0x153810 --refs
...
def dossier(rva: int, refs: bool = False, depth: int = 0) -> tuple[list[str], int]:
    ...
    if refs and nrefs:
        from gruntz.sema.xref import references
...
def main(argv: list[str] | None = None) -> int:
    import argparse
    ap = argparse.ArgumentParser(prog="gruntz sema rva",
