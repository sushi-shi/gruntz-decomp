"""gruntz.sema.map - the retail address-space map.
    ...
    python3 -m gruntz.sema.map                     # whole-image overview
    python3 -m gruntz.sema.map at 0x153810         # the row here, with neighbours
    python3 -m gruntz.sema.map range 0x153000 0x154000
    python3 -m gruntz.sema.map gaps --min 64       # unclaimed .text runs
    python3 -m gruntz.sema.map units --top 30      # per-unit contribution
    python3 -m gruntz.sema.map find CImage::Blit
...
def main(argv: list[str] | None = None) -> int:
    import argparse
    ap = argparse.ArgumentParser(prog="gruntz sema map",
