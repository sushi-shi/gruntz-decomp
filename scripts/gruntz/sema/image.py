"""gruntz.sema.image - the retail image, scanned for address queries.

Three whole-image scans sit on top of gruntz.core.pe, shared by every sema
view and computed once per process:

  * the .reloc HIGHLOW fixup map - every DIR32 site in the image paired with
...
Nothing here knows what a label MEANS - that is gruntz.sema.index's job.
"""
...
from gruntz.core.pe import Pe, image as _image
...
class Image:
    ...
    def reloc(self) -> dict[int, int]:
        """{fixup_site_rva: target_rva} for every IMAGE_REL_BASED_HIGHLOW site.
