...
def _parse_manifest(text):
    """<unit> \\t <count> lines -> {unit: count}. Comments/blanks ignored."""
    out = {}
...
def _committed_manifest():
    """The manifest as of HEAD, read from git - NOT from the working tree.

    The working-tree copy is written by this very function, so once a build has
    regenerated it at a lower count every later build compares the loss against
    the loss. Only a value the build cannot have produced is a floor. Returns
    (counts, source) where source is 'HEAD' or 'worktree' (the degraded fallback
    used when git is unavailable - an exported source tree, say).
    """
    try:
        out = subprocess.run(
            ["git", "-C", str(REPO), "show",
             f"HEAD:{LABELS_MANIFEST.relative_to(REPO).as_posix()}"],
            capture_output=True, text=True, timeout=30)
        if out.returncode == 0:
            return _parse_manifest(out.stdout), "HEAD"
...
def _ack_units():
    """GRUNTZ_LABELS_ACK -> the set of units whose drop is acknowledged.

    Unit names, comma- or space-separated. The old blanket `=1` is REJECTED: a
    global kill-switch exported once in a shell disables the gate for every unit
    forever and leaves no trace, which is exactly how a 75 -> 74 loss in
    `gamelevel` went unnoticed for a whole session. Naming the units means a
    stale export cannot hide a NEW loss in a DIFFERENT unit.
    """
    raw = (os.environ.get("GRUNTZ_LABELS_ACK") or "").strip()
    if not raw:
        return set(), None
    names = {t for t in re.split(r"[,\s]+", raw) if t}
    if names & {"1", "all", "yes", "true"}:
        return set(), ("GRUNTZ_LABELS_ACK must NAME the units whose drop is "
                       "deliberate (GRUNTZ_LABELS_ACK=gamelevel,grunt), not "
                       f"'{raw}'. A blanket acknowledgement is a silent global "
                       "kill-switch - it is what let a labelled-function loss "
                       "pass unnoticed - so it is no longer accepted.")
    return names, None
...
def check_labels_manifest(rows):
    ...
    nothing. This is the last line: a per-unit function count. Any unit whose count
    DROPS - or that disappears entirely - fails the build and is named, with the
    before/after. Growth is fine and is recorded silently.

    Why the floor is read from git: the failure we are guarding against makes the
    numbers look BETTER (fewer functions in the denominator => a higher exact %).
    Nothing in the build itself can notice that, because every metric is computed
    from the very file that just lost the rows - INCLUDING this manifest, which the
    build regenerates. So the floor is the elementwise max of HEAD's copy and the
    working-tree copy: HEAD is a value no build could have written, and the max also
    holds a within-session gain that has not been committed yet.

    A deliberate drop (deleting a stub, moving functions between units) is
    acknowledged by NAMING the units: GRUNTZ_LABELS_ACK=gamelevel,grunt. That
    rewrites the manifest; the commit then RECORDS the drop, which is exactly the
    "explicit acknowledgement" we want in the history.
    """
    cur = {}
    ...
    ack, ack_err = _ack_units()
    if ack_err:
        log(f"ERROR {ack_err}")
        return 1

    head, src = _committed_manifest()
    work = _parse_manifest(LABELS_MANIFEST.read_text()) if LABELS_MANIFEST.exists() else {}
    if head is None:
        log(f"WARN {LABELS_MANIFEST.relative_to(REPO)}: cannot read HEAD's copy "
            f"(no git?) - falling back to the working-tree copy, which this build "
            f"rewrites, so the floor is only as good as the last build.")
        floor = work
    else:
        floor = {u: max(head.get(u, 0), work.get(u, 0)) for u in set(head) | set(work)}

    drops = [(u, n, cur.get(u, 0)) for u, n in sorted(floor.items())
             if cur.get(u, 0) < n]
    unacked = [d for d in drops if d[0] not in ack]
    if unacked:
        for u, was, now in unacked:
            log(f"ERROR unit '{u}': labelled functions DROPPED {was} -> {now}"
                + ("  (the unit vanished entirely)" if now == 0 else ""))
        lost = sum(was - now for _u, was, now in unacked)
        log(f"{len(unacked)} unit(s) lost {lost} labelled function(s) vs the "
            f"{src} copy of {LABELS_MANIFEST.relative_to(REPO)}. A shrinking "
            f"denominator makes every metric look BETTER while the tree gets WORSE, "
            f"so this is fatal. If the drop is DELIBERATE, re-run with "
            f"GRUNTZ_LABELS_ACK="
            + ",".join(u for u, _w, _n in unacked)
            + " to rewrite the manifest, and COMMIT it - the floor is HEAD's copy, "
              "so an uncommitted acknowledgement does not stick.")
        return 1
    for u, was, now in drops:
        log(f"ACK unit '{u}': labelled functions {was} -> {now} (acknowledged)")
    ...
    new = ("# labels_manifest.tsv - per-unit labelled-FUNCTION counts, the committed\n"
           ...
           "# count DROPS fails the build; see check_labels_manifest(). The floor is\n"
           "# read from HEAD (git), not from this file, so regenerating it cannot lower\n"
           "# it. Acknowledge a deliberate drop by NAMING the units:\n"
           "#   GRUNTZ_LABELS_ACK=<unit>[,<unit>...]   (a blanket '1' is rejected)\n"
    ...
    if not LABELS_MANIFEST.exists() or LABELS_MANIFEST.read_text() != new:
        ...
        log(f"labels manifest: {len(cur)} unit(s), {sum(cur.values())} function(s)"
            + (f"  [ACK: {','.join(sorted(ack))}]" if ack else ""))
    return 0
