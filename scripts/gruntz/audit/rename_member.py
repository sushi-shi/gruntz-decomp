#!/usr/bin/env python3
"""gruntz.audit.rename_member - type-aware bulk member renamer via clangd LSP.
...
them.
  ...
  python3 -m gruntz.audit.rename_member --class CGameObject m_5c=m_screenX m_60=m_screenY

  # or a mapping file (one `old=new` or `old new` per line, # comments allowed)
  python3 -m gruntz.audit.rename_member --class CGameObject --map cgo.map

  # preview only (no writes): per-field + per-file edit counts
  python3 -m gruntz.audit.rename_member --class CGameObject --map cgo.map --dry-run
