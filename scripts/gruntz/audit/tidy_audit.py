...
Usage:
    python -m gruntz.audit.tidy_audit                 # whole src/ tree
    python -m gruntz.audit.tidy_audit src/Gruntz      # a subtree
    python -m gruntz.audit.tidy_audit --csv hacks.csv # file,line,check,message
    python -m gruntz.audit.tidy_audit --top 40 -j 24

Run inside `nix develop` (any shell - it only needs clang-tidy + the compdb).
Also exposed as `gruntz audit tidy`.
"""
