...
RM_START = "<!-- link-score:start -->"
RM_END = "<!-- link-score:end -->"


def link_block(R, C):
    """The README's whole-linked-image scoreboard.

    objdiff answers "does each OBJECT match the object we carved out of retail".
    This answers a different question - "is the whole PROGRAM the shape retail
    shipped" - and the two genuinely disagree: every object can score well while
    the image is tens of KB short because unclaimed code was never carved at all.
    Neither number subsumes the other, so the README carries both.
    """
    import datetime
    import hashlib
    rs = {s['name']: s for s in R.sections}
    cs = {s['name']: s for s in C.sections}
    rtot, ctot = RETAIL.stat().st_size, CAND.stat().st_size
    dg = hashlib.sha256(CAND.read_bytes()).hexdigest()[:12]
    stamp = datetime.datetime.fromtimestamp(
        CAND.stat().st_mtime).strftime("%Y-%m-%d %H:%M")

    L = [RM_START, "## Link status", "",
         "_Auto-generated by `python -m gruntz.audit.link_sections --write-readme`, "
         "which `ninja candidate` runs; do not hand-edit. **`gruntz build` does NOT "
         "link**, so this block is only as fresh as the last `ninja candidate` - the "
         "candidate's digest and timestamp are stated below so a stale block is "
         "visible rather than silent._", "",
         f"**Candidate image: {ctot:,} B vs retail {rtot:,} B "
         f"({100.0 * ctot / rtot:.2f}%, {ctot - rtot:+,} B).**", ""]
...
def write_readme(block):
    ...
    if RM_START in text and RM_END in text:
        ...
        readme.write_text(pre + block.rstrip("\n") + post)
    else:                                   # first install: after the match block
        anchor = "<!-- match-score:end -->"
...
def main(argv=None):
    ...
    ap.add_argument('--write-readme', action='store_true',
                    help="refresh the README's link-status block and exit")
    ...
    # --write-readme needs only the two images; everything else needs the .map too
    need = (RETAIL, CAND) if a.write_readme else (RETAIL, CAND, CMAP)
    for p in need:
        if not p.exists():
    ...
    if a.write_readme:
        write_readme(link_block(R, C))
        print(f"[link-sections] README link-status refreshed "
              f"({CAND.stat().st_size:,} B candidate)")
        return 0
