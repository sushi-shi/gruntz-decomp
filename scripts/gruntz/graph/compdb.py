"""gruntz.graph.compdb - units.toml -> build/clangd/compile_commands.json.

    python3 -m gruntz.graph.compdb            # (re)generate + coverage check
...
at build/clangd/), gruntz.tool.clang (per-TU extraction flags - a unit with
no entry silently falls back to bare MS flags, which is why generation always
ends with a coverage check THROUGH the consumer's own parser), the gruntz.lsp
verbs, and gruntz.verify.fingerprints.

Mechanics kept from the proven frozen generator (scripts/gruntz-old/init/
clangd.py):
...
cannot see - after re-pinning, run `python3 -m gruntz.graph.compdb` once (the
...
from gruntz.core.paths import BUILD, INCLUDE, REPO, VENDOR, dxsdk_dir, msvc_dir
...
def resolve_include_dirs() -> tuple[Path, Path, str]:
    ...
    .#gruntz-toolchain and reads the include dirs off the store path.
    """
    try:
    ...
    try:
        out = subprocess.check_output(
            ["nix", "build", ".#gruntz-toolchain", "--no-link",
        ...
        raise SystemExit(
            ...
            f"`nix build .#gruntz-toolchain` works.\n  cause: {e}") from e
    root = Path(out[-1])
