"""gruntz.graph.link - PHASE 2: base objs -> candidate GRUNTZ.EXE + .map.

    python3 -m gruntz.graph.link [--out E] [--objs-dir D] [--res R] [--order F]

Graph phase 2, opt-in (`ninja candidate` / `gruntz link`): the genuine VC5
...
It also blocks `/INCREMENTAL` (LNK4075), and retail IS an incremental link. A
...
let them fire - what the devs' link did. Three groups declare themselves nowhere
and are named explicitly, all corroborated by retail's own import table:
version + winmm (imported by the game, requested by no header), DirectX 6 (the
SDK ships no `#pragma comment(lib)`), and mss32 + smackw32, whose import libs
are SYNTHESISED from retail's import table by gruntz.graph.implib.
...
must not survive is the PATCHING behaviour: with a live `.ilk`, link.exe edits
the previous image instead of building one, and an incremental pass does not
re-bind the `.res` - so a rebuild once emitted a binary with no `.rsrc` at all
(and every MFC dialog is created from a DIALOG resource). A link whose output
depends on what happened to be left on disk is not reproducible, so the prior
image and its incremental database are dropped first.
"""
...
#: IMPORT-DESCRIPTOR ORDER (all 16 DLLs, 0/120 inversions). Two rules produce
...
LINK_LIBS = ["nafxcw.lib", "libcmt.lib",
             ...
             "comctl32.lib", "mss32.lib", "winmm.lib", "dplayx.lib",
...
#: The engine projects (`C:\\Proj\\{DDrawMgr,DinMgr2,Dsndmgr,NetMgr}` plus the
#: shared Bute/Rez/Image/Wwd/Utils/Crypto code and vendored zlib) shipped as
#: static LIBRARIES, not as objects on the link line. Retail proves it: MSVC's
#: incremental linker emits an E9 thunk for every cross-OBJECT call but never
#: for a `.lib` member, and retail's thunk targets stop dead at 0x11c860.
#: There were at least TWO archives: inside the library region the vendored zlib
#: units form a perfectly contiguous run after the engine block - the signature
#: of a second library searched after the first - and a third block sits after
...
def candidate(out: Path, objs_dir: Path, *, mapfile: Path | None = None,
              ...
              engine_lib: bool = False, incremental: bool = True,
    ...
    from gruntz.tool import link as link_tool
    ...
    rsp_lines = [
        ...
        f"/ENTRY:{ENTRY}",
        # Retail HAS a .reloc (it is why the image is delinkable at all), so it
        # was linked /FIXED:NO. Purely additive: .text/.rdata/.data come out
...
def main() -> int:
    ...
    ap.add_argument("--engine-lib", action="store_true",
                    help="archive the engine modules and link THOSE, as retail "
                         "did (incremental thunks 4559 -> 2976 vs retail's 2695, "
                         "because a .lib member never gets one)")
    ap.add_argument("--no-incremental", action="store_true",
                    help="/INCREMENTAL:NO - the flat layout. Default is YES "
                         "because retail is an incremental link; use this only "
