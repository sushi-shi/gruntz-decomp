"""gruntz.graph.implib - synthesise the import LIBs the toolchain does not ship.

    python3 -m gruntz.graph.implib            # synthesise every missing lib
    python3 -m gruntz.graph.implib --list     # report coverage, build nothing

Retail GRUNTZ.EXE load-time-imports 16 DLLs. Fourteen have a real import lib in
the toolchain - the Win32 set in `msvc/lib` and DirectX 6 in `dx/Lib`. The two
with no lib anywhere are the RAD Game Tools SDKs the devs had and we do not:
`mss32.dll` (Miles Sound System) and `smackw32.dll` (Smacker video). Only their
HEADERS are vendored, so every call to them is an unresolved external at link.
...
from gruntz.core.paths import BUILD, dxsdk_dir, msvc_dir
from gruntz.core.pe import Pe, image
from gruntz.tool import ToolError
from gruntz.tool.wine import find_ci, winepath
...
def stub_source(dll: str, names, hints: dict[str, int] | None = None) -> str:
    ...
    lines = [f"/* GENERATED by gruntz.graph.implib - stub exports for {dll}.",
...
def synthesize(dll: str, hints: dict[str, int], out_dir: Path = OUT_DIR,
    ...
    from gruntz.tool import cl, link
    from gruntz.tool.wine import era_tool
