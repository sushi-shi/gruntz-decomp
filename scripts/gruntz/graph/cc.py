...
Both scratch paths are per-PROCESS. Two `gruntz build` runs in one tree (an
...
scratch names used to be shared:
  ...
  * a shared `<unit>.obj.install` is worse, because it makes the rename stop
    ...
    gruntz.compare.normalize failed a real build with "COFF string table is
...
def main(argv: list[str] | None = None) -> int:
    ...
    ap = argparse.ArgumentParser(
        prog="gruntz.graph.cc", description=__doc__,
