"""Derive the semantic-abstraction queue for every current sub-100 function.
    ...
    gruntz walls abstractions [--todo] [--unit U] [--module M] [--level L]
...
from gruntz.verify.readme import unit_modules
...
def build(unit: str | None = None, modules: set[str] | None = None,
    ...
    from gruntz.permute.campaign import classified_candidates
    from gruntz.walls.reviews import current as current_reviews, load as all_reviews
...
def main(argv=None) -> int:
    ...
    ap = argparse.ArgumentParser(
        prog="gruntz walls abstractions", description=__doc__,
    ...
    if stale_units:
        ...
        print("           hash-scoped terminal reviews in those units cannot be excluded; "
              "run `gruntz verify fingerprints` (or a full build)")
