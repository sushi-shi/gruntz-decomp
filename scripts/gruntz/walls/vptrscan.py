"""gruntz.walls.vptrscan - the VPTR-STAMP census.
...
def _injected(unit: str = "ddrawworkerhost") -> int:
    ...
    def run(copy: bytes):
        with tempfile.TemporaryDirectory(prefix="gruntz-vptrscan-") as tmp:
            root = Path(tmp)
            (root / "base").mkdir()
            (root / "target").mkdir()
...
def main(argv=None) -> int:
    ap = argparse.ArgumentParser(prog="gruntz walls vptrscan",
