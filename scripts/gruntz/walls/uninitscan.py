"""gruntz.walls.uninitscan - the CONDITIONALLY-UNINITIALIZED LOCAL sieve.
...
already proven faithful by the default-arm evidence.
    ...
    gruntz walls uninitscan [--file SUBSTR] [--control] [--all]
"""
...
def entries():
    if not DB.is_file():
        print(f"[uninitscan] no compilation database at {DB} - run "
              "`gruntz build` (it emits the clangd database)", file=sys.stderr)
        raise SystemExit(2)
...
def main(argv=None) -> int:
    ...
    ap = argparse.ArgumentParser(
        prog="gruntz walls uninitscan", description=__doc__,
