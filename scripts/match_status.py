...
def cmd_update(args) -> int:
    ...
    if stale:
        print(f"WARNING: fingerprint cache stale/absent for {len(stale)} unit(s) "
              ...
              f"(not refreshed). Run `gruntz build` / `func_fingerprints.py` first for "
...
def classify(cur, base_funcs, fp):
    ...
    for key, prev in sorted(base_funcs.items()):
        if key in cur:
            continue
        unit, fn = key
        # vanished: a real edit to its source -> likely renamed/removed (REMOVED);
        # otherwise a genuine loss of a previously-matched function (LOST, gated).
        yield ("REMOVED" if real_edit(prev["fp"], fp(*key)) else "LOST",
...
def cmd_check(args) -> int:
    ...
    if stale:
        print(f"WARNING: fingerprint cache stale/absent for {len(stale)} unit(s) "
              ...
              f"`scripts/func_fingerprints.py` to refresh.", file=sys.stderr)
