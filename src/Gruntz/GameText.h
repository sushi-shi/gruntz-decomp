...
//
//   GetWorldDisplayName    @0x82990  - seeds the 8 world display names.
//   GetEndLevelStatLabels  @0x18740  - seeds the 8 end-of-level stat labels.
//   GetWarlordName         @0x1ec20  - boss/warlord name by id (returns CString).
//   CContainerErr::CContainerErr @0x16d9c0 - the container-library exception
...
// const-char* ctor (CString::CString(const char*), NAFXCW @0x1b9d4c) is used by
...
// whose vtable lives @0x5f04cc; Ghidra's "__non_rtti_object" is an FID guess).
...
//   +0x00  m_vtbl : void*  - the vtable pointer (0x5f04cc).
...
// ---------------------------------------------------------------------------
extern void *g_containerErrVtbl;   // 0x5f04cc

class CContainerErr {
public:
    CContainerErr(const char *msg);      // @0x16d9c0
public:
    void       *m_vtbl;  // +0x00  the vtable pointer (0x5f04cc)
