...
// teardown), then the CObArray member destructs and ~CWapObject folds in to
...
// ~CAniRecordArray (trylevel 0) and ~CWapObject grand-base re-stamp fold in. /GX frame
// from the destructible base + member. (eh-dtor-implicit-vptr-stamp-first.md.)
RVA(0x00152e30, 0x53)
CAniElement::~CAniElement() {
    DeleteAll();
    // m_items.~CAniRecordArray() (trylevel 0) + ~CWapObject() (grand-base restore) fold here.
}
