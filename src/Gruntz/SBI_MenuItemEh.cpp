#include <rva.h>
#include <Ints.h>
// SBI_MenuItemEh.cpp - the /GX EH-framed CSBI_MenuItem destructor (C:\Proj\Gruntz). The split
// off the frameless base TU is matching-neutral (each function is RVA-keyed).
//
// REAL polymorphic hierarchy:  CSBI_MenuItem : CSBI_Image : CSBI_RectOnly : CStatusBarItem.
// Each base subobject has a non-trivial (inline) virtual dtor, so MSVC folds the
// base teardown chain into the most-derived ~CSBI_MenuItem and emits the full /GX SEH
