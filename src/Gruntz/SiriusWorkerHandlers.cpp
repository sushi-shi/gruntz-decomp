...
struct Worker {
    ...
    i32 m_1c;                  // +0x1c  state tag (switch key)
};
...
// ---------------------------------------------------------------------------
// @early-stop
// reloc-masked plateau (97.86%): the .text is BYTE-IDENTICAL to retail (the
// instruction diff is empty). The residual is four reloc OPERAND-NAME mismatches
// my clean C++ base cannot reproduce - all reloc-masked, code bytes match:
//   operator new ??2@YAPAXI@Z   vs the delinked _RezAlloc (0x1b9b46; same addr,
//                               the engine allocator MSVC's new wraps),
//   ??0SubRecord54@@...         vs Stub_03fc70 (the bare delinker name),
//   _Worker_DefaultPump         vs FUN_0056e4f0 (a Ghidra placeholder),
//   the /GX scope table $L###    vs Unwind@005d9d50 (the delinked SEH handler).
// See reloc-typing-vptr-global.md / external-nobody-callee.md. Defer to the
// final sweep (re-attackable only if the delinker emits matchable callee names).
RVA(0x0003d670, 0xf1)
...
// @early-stop
// reloc-masked plateau (97.86%): .text byte-identical to retail; same four
// reloc operand-name mismatches as Handler03d670 (sub-record is SubRecord5C /
// CGruntSelectedSprite, 0x5c bytes). Defer to the final sweep.
RVA(0x0007db20, 0xf1)
...
// @early-stop
// reloc-masked plateau (97.86%): .text byte-identical to retail; same four
// reloc operand-name mismatches as Handler03d670 (sub-record is SubRecord60 /
// CGruntToySprite, 0x60 bytes). Defer to the final sweep.
RVA(0x0007dda0, 0xf1)
