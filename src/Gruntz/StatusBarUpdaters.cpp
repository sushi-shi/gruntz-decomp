...
struct CRegHolder {
    char m_pad00[0x24];
    struct M24 { char m_pad00[0x5c]; CTileGrid *m_5c; } *m_24;   // +0x24 -> +0x5c grid
...
void EngineLabelBacklog::UpdateGruntOvenStatusBar()
{
    ...
    do {
        if (tab->m_0 == 1) {
            ...
            unsigned int delay = g_buteMgr.GetDwordDef(
                "StatusBar", "GruntOvenDelay", 0xc8);
...
void EngineLabelBacklog::UpdateDestructButtonStatusBar()
{
    ...
    case 1: {
        ...
        if (d >= *(__int64 *)&b->m_568) {
            ...
            b->m_568 = g_buteMgr.GetDwordDef(
                "StatusBar", "DestructButtonWarningDelay", 0x32);
            b->m_56c = 0;
            b->m_560 = g_645588;
            b->m_564 = 0;
...
void EngineLabelBacklog::UpdateChipGrinderStatusBar()
{
    ...
    if (m[0x4e8 / 4] == 1 || m[0x4e8 / 4] == 2) {
        unsigned int delay = g_buteMgr.GetDwordDef(
            "StatusBar", "FallingItemDelay", 0x32);
        ...
        if (m[0x508 / 4] >= 0x1c7) {
            ...
            delay = g_buteMgr.GetDwordDef(
                "StatusBar", "FallingItemShredderDelay", 0x64);
...
void EngineLabelBacklog::LoadSwitchDownSprite()
{
    ...
    int px = (m_8 << 5) + 0x10;
    int py = (m_c << 5) + 0x10;
    if (px < g_gameReg->m_144 && px >= g_gameReg->m_13c
        && py < g_gameReg->m_148 && py >= g_gameReg->m_140) {
        CStatusBarHolder *h = g_gameReg->m_30->m_28;
        if (h->m_30 == 0) {
            CSprite *spr = 0;
...
int EngineLabelBacklog::UpdateWarpStoneStatusBar(int a0, int phase, int srcX, int srcY)
{
    ...
    CSprite *spr = 0;
    int n = phase + 1;
    g_gameReg->m_30->m_28->m_10map.Lookup(
        "GAME_STATUSBAR_TABZ_GAMETAB_WARP", &spr);
    int *frame = (spr && n >= spr->m_64 && n <= spr->m_68)
                 ? spr->m_14[n] : 0;
    m[0x38 / 4] = (int)frame;
    if (frame == 0)
        return 1;
    ...
    int cx, dy;
    switch (phase) {
    case 2:  cx = 0x69; dy = 0x26; break;
    case 3:  cx = 0x65; dy = 0x50; break;
    case 4:  cx = 0x69; dy = 0x54; break;
    default: cx = 0x34; dy = 0x29; break;
    }
