...
// The grunt-indicator / HUD sprite leaves the pump `new`s (0x7dc60..0x7e2a0), as thin
// size-views in a shared header (their canonical class headers pull the Grunt.h world,
// which cannot coexist with the canonical UserLogic.h this TU uses).
#include <Gruntz/AnimWorkerSpriteLeaves.h>
...
//
// The 0x7dc60..0x7e2a0 handlers are byte-identical to the three below bar the leaf
// TYPE `new`d on state 0 (hence the size + ctor target); shared as a macro. The three
// original handlers stay written out above/below in RVA order.
...
// key rec->m_1c is UNSIGNED (u32) so MSVC5 emits the range checks as unsigned
// ja/jbe, matching retail byte-for-byte (a signed i32 key emits jg/jle and caps
// each at 97.86%; see docs/patterns/switch-key-unsigned-ja-vs-jg.md).
RVA(0x0003d2b0, 0xf1)
...
RVA(0x0007e2a0, 0xf1)
i32 Handler07e2a0(Owner* owner) { ANIM_WORKER_PUMP(CGruntPowerupSprite) } // new 0x60, ctor 0x07fdb0
RVA(0x00046990, 0xf1)
