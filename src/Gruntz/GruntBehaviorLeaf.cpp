...
i32 CGruntBehaviorLeaf::LoadGruntDecayConfig() {
    ...
    if (mode == 1 || mode == 2 || mode == 0xb || mode == 6) {
        ...
        i64 e = static_cast<i64>(static_cast<u32>(g_frameTime))
                - *reinterpret_cast<i64*>(&m_decayTimerLo);
...
i32 CGruntBehaviorLeaf::LoadGruntDecayConfig2() {
    if (static_cast<i64>(static_cast<u32>(g_frameTime)) - *reinterpret_cast<i64*>(&m_decayTimerLo)
        >= *reinterpret_cast<i64*>(&m_decayDurationLo)) {
    ...
    i64 e =
        static_cast<i64>(static_cast<u32>(g_frameTime)) - *reinterpret_cast<i64*>(&m_decayTimerLo);
    u32 elapsed = e < 0 ? 0 : static_cast<u32>(e);
    i32 r = static_cast<i32>(
        (static_cast<double>(elapsed) * 256.0
         / static_cast<double>(g_buteMgr.GetDwordDef("Grunt", "DecayTime", 0xbb8)))
