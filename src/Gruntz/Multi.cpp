...
i32 CMulti::ShowMultiStartDlg() {
    ...
    if (r != 1) {
        if (m_isHost != 0) {
            ...
            rec->m_liveGate = 0;
            ChannelSlots_Set(rec->m_colorIndex, 1);
...
i32 CMulti::DispatchRecvMsg(i32 sender, char* buf, i32 size) {
    ...
    switch (msg->m_msgId) {
        ...
        case 0x41f:
            SendStatValue(sender, 0x420, msg->m_value, 0);
            break;
...
i32 CMulti::OnPlayerLeft(i32 playerId) {
    ...
    slot->m_liveGate = 0;
    ChannelSlots_Set(slot->m_colorIndex, 1);
...
i32 CMulti::ParseChannelTable(void* packet) {
    ...
    for (i32 i = 0; i < 4; i++) {
        ...
        if (ch != 0) {
            ...
            ch->m_colorIndex = p->m_rows[i].m_colorIndex;
            ...
            if (m_isHost == 0 && ch->m_liveGate != 0) {
                ChannelSlots_Set(ch->m_colorIndex, 0);
            }
        }
    }
    return 1;
...
i32 CMulti::RemoveChannel(i32 idx) {
    ...
    ch->m_liveGate = 0;
    ChannelSlots_Set(ch->m_colorIndex, 1);
    return 1;
}
...
i32 CMulti::ParseOneChannel(void* rec) {
    ...
    ch->m_name = r->m_name;
    ch->m_colorIndex = r->m_colorIndex;
