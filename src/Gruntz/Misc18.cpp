...
struct FreeOwner {
    char pad00[0x1c];
    FreeNode* m_node; // +0x1c
    char pad20[0x24 - 0x20];
    void(__cdecl* m_freeFn)(void*, void*); // +0x24 free callback
    void* m_freeCtx;                       // +0x28 callback context
};

// @early-stop
// regalloc + bool-tail idiom (~76%): logic correct; retail materializes the
// (tag==0x71)?-3:0 via setne/dec/and and reloads self->m_node per field; the cl
...
i32 RecFree(FreeOwner* self) {
    ...
    if (self->m_node->m_08) {
        self->m_freeFn(self->m_freeCtx, self->m_node->m_08);
    }
    if (self->m_node->m_38) {
        self->m_freeFn(self->m_freeCtx, self->m_node->m_38);
    }
    if (self->m_node->m_34) {
        self->m_freeFn(self->m_freeCtx, self->m_node->m_34);
    }
    if (self->m_node->m_2c) {
        self->m_freeFn(self->m_freeCtx, self->m_node->m_2c);
    }
    FreeNode* n = self->m_node;
    i32 tag = n->m_typeTag;
    self->m_freeFn(self->m_freeCtx, n);
    self->m_node = 0;
    return tag == 0x71 ? -3 : 0;
}
...
struct BitBuf {
    char pad00[8];
    u8* m_out; // +0x08 output stream
    char pad0c[0x10 - 0x0c];
    i32 m_writePos; // +0x10 byte write index
    ...
    i32 m_bitCount; // +0x16b4 pending bit count
};

// @early-stop
// regalloc tie (~84%): logic byte-exact; retail caches m_out in esi across the two
// byte stores where this cl reloads it.
RVA(0x0018a190, 0x8c)
void BitFlush(BitBuf* b) {
    i32 n = b->m_bitCount;
    if (n == 0x10) {
        b->m_out[b->m_writePos] = b->m_accLo;
        b->m_writePos++;
        b->m_out[b->m_writePos] = b->m_accHi;
        b->m_writePos++;
        *(u16*)&b->m_accLo = 0;
        b->m_bitCount = 0;
    } else if (n >= 8) {
...
struct Coder {
    ...
    u16* m_buf;   // +0x38 work buffer
    i32 m_3c;     // +0x3c
    i32 m_length; // +0x40 length
    ...
    i32 m_70;        // +0x70
    i32 m_74;        // +0x74
    i32 m_modeIndex; // +0x78 mode index
...
void CoderReset(Coder* c) {
    c->m_30 = c->m_20 << 1;
    c->m_buf[c->m_length - 1] = 0;
    memset(c->m_buf, 0, c->m_length * 2 - 2);
    i32 i = c->m_modeIndex * 0xc;
