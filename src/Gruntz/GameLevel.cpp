...
i32 CGameLevel::Unload() {
    ...
    for (i = 0; i < m_planes.GetSize(); i++) {
        CLevelPlane* child = reinterpret_cast<CLevelPlane*>(m_planes.GetData()[i]);
        if (child) {
...
i32 CGameLevel::SetExtentsAndBuildAll(i32 w, i32 h) {
    ...
    if (m_planes.GetSize() > 0) {
        do {
            (reinterpret_cast<CLevelPlane*>(m_planes.GetData()[i]))->Build(&rect);
            ++i;
...
void CGameLevel::SyncToMainIndex(void* visitor) {
    ...
    if (m_mainIndex >= 0) {
        do {
            (reinterpret_cast<CPlaneRender*>(m_planes.GetData()[i]))->Draw(static_cast<CPlaneDrawCtx*>(visitor)); // 0x162010
            ++i;
        } while (i <= m_mainIndex);
    }
}
...
void CGameLevel::VisitVisible(void* visitor, CDDrawChildGroup* ctx) {
    ...
    if ((m_08 & 1) && chain != 0 && (m_planes.GetSize() > 0 ? m_planes.GetData()[0] : 0) != 0) {
        ...
        if (m_planes.GetSize() > i) {
            do {
                CLevelPlane* p =
                    (i >= 0 && i < m_planes.GetSize()) ? reinterpret_cast<CLevelPlane*>(m_planes.GetData()[i]) : 0;
                ...
                (reinterpret_cast<CLevelPlane*>(m_planes.GetData()[i]))->Draw(static_cast<CPlaneDrawCtx*>(visitor));
                ++i;
            } while (i < m_planes.GetSize());
        }
    ...
    if (m_mainIndex >= 0) {
        do {
            (reinterpret_cast<CLevelPlane*>(m_planes.GetData()[idx]))->Draw(static_cast<CPlaneDrawCtx*>(visitor));
            ++idx;
    ...
    if (j < m_planes.GetSize()) {
        do {
            (reinterpret_cast<CLevelPlane*>(m_planes.GetData()[j]))->Draw(static_cast<CPlaneDrawCtx*>(visitor));
            ++j;
