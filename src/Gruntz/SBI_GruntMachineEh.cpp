#include <rva.h>
#include <Ints.h>
// SBI_GruntMachineEh.cpp - the /GX EH-framed CSBI_GruntMachine destructor (C:\Proj\Gruntz). The split
// off the frameless base TU is matching-neutral (each function is RVA-keyed).
//
// REAL polymorphic hierarchy:  CSBI_GruntMachine : CStatusBarItem.
// Each base subobject has a non-trivial (inline) virtual dtor, so MSVC folds the
// base teardown chain into the most-derived ~CSBI_GruntMachine and emits the full /GX SEH
