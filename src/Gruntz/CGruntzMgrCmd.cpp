...
namespace GruntzMgrCmd {
    ...
    struct GZGateB { char _p[0x30]; i32 m_30; };
    struct GZGateA { char _p[0x28]; GZGateB* m_28; };
    struct GZSound { void Play(i32 a, i32 b, i32 c, i32 d); }; // 0x1f940 __thiscall
    ...
    struct GZWndSub { char _p0[4]; void* m_4; };
    struct CGruntzMgr {
        char _p0[0x4];
        GZWndSub* m_04; // 0x04
        char _p8[0xc - 0x8];
    ...
    struct GZObj58 { void Set58(i32 n); void Reset58(); }; // 0x4408 / 0x3463

#define PLAYCUE(TAG)                                                              \
    if (m_30->m_28->m_30 == 0) {                                                  \
        GZSound* _c = CueLookup(TAG);                                             \
        if (_c) _c->Play(g_sndCueTag, 0, 0, 0);                                   \
    }
...
#define WARP(N, ERR)                                                              \
    ...
    i32 CGruntzMgr::HandleCommand(i32 p1, i32 nID, i32 p3) {
            ...
            if (m_2c->vf10() == 3) {
                ...
                case 0x8131:
                    ...
                    if (!BrickPickup(0x3a))
                        return 0;
                    PLAYCUE("GAME_MAJORCHEAT");
                    AppendChat("Ladies and gentlemen, please welcome... death... "
                ...
                case 0x8246: {
                    ...
                    AppendChat("My name is Kevin Lambert.  You typed in my cheat "
                               "code.  Prepare to die.");
                    return 1;
                }
                ...
                case 0x81a9:
                    ...
                    AppendChat("They should call you Cheat Cheatelson from "
        ...
        case 0x8008: {
            i32 st = m_2c->vf10();
            if (st == 9 || st == 0xd || st == 0xf || st == 0xe || st == 8 ||
                st == 0xa || st == 0x12 || st == 0x11)
                return 1;
            if (Dispatch(9, 1, 1, 0))
                return 1;
