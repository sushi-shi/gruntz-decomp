...
// the source/dest boxes, derives the per-cell step (dx = box->h / m_columns, dy =
// box->w / m_rows) and the cell radius (sqrt(dx*dx+dy*dy)), then walks an
// (m_rows x m_columns) grid emitting one projected mesh point per cell into the growable
...
// sub-rects (pt48 = src cell, pt64 = dst cell), assembled (order gated on m_recordOrder)
...
struct FxConfig {
    i32 m_00;
    FxBox* m_srcBox; // +0x04 src box override (else this->m_srcBox)
    FxBox* m_dstBox; // +0x08 dst box override (else this->m_dstBox)
    i32 m_0c;
...
public:
    ...
    char m_00[0x24];
    FxBox* m_srcBox; // +0x24 default src box
    FxBox* m_dstBox; // +0x28 default dst box
    char m_2c[0x3c - 0x2c];
    FxBox* m_activeSrcBox; // +0x3c active src box
    FxBox* m_activeDstBox; // +0x38 active dst box
    i32 m_40;              // +0x40
    i32 m_44;              // +0x44
    i32 m_48;              // +0x48
    i32 m_recordOrder;     // +0x4c record-order flag
    i32 m_columns;         // +0x50 columns
    i32 m_rows;            // +0x54 rows
    FxPointArray m_mesh;   // +0x58 mesh-point array
};

// ===========================================================================
// 0x17ea00 - Build the (m_rows x m_columns) ellipse mesh into this->m_mesh.
// ===========================================================================
// @early-stop
...
// OffsetRect-accumulated sub-rects, the m_recordOrder-gated 40-byte record, the inlined
...
i32 CFxModeT3::Build(FxConfig* cfg) {
    m_activeSrcBox = cfg->m_srcBox ? cfg->m_srcBox : m_srcBox;
    m_activeDstBox = cfg->m_dstBox ? cfg->m_dstBox : m_dstBox;
    if (cfg->m_10 == 0) {
        return 0;
    }
    m_40 = cfg->m_0c;
    m_44 = cfg->m_10;
    m_48 = cfg->m_18;
    m_recordOrder = cfg->m_recordOrder;
    m_columns = cfg->m_columns;
    m_rows = cfg->m_rows;

    m_mesh.Init(0, -1);

    i32 halfW = m_activeSrcBox->m_width / 2;
    i32 halfH = m_activeSrcBox->m_height / 2;
    i32 dx = m_activeDstBox->m_height / m_columns;
    i32 dy = m_activeDstBox->m_width / m_rows;
    ...
    for (i32 row = 0; row < m_rows; row++) {
        ...
        for (i32 col = 0; col < m_columns; col++) {
            ...
            i32 idx = m_mesh.m_nSize;
            i32 newSize = idx + 1;
            if (newSize == 0) {
                if (m_mesh.m_pData) {
                    RezFree(m_mesh.m_pData);
                    m_mesh.m_pData = 0;
                }
                m_mesh.m_nMaxSize = 0;
                m_mesh.m_nSize = 0;
            } else if (m_mesh.m_pData == 0) {
                m_mesh.m_pData = (FxPoint*)RezAlloc(newSize * sizeof(FxPoint));
                memset(m_mesh.m_pData, 0, newSize * sizeof(FxPoint));
                m_mesh.m_nMaxSize = newSize;
                m_mesh.m_nSize = newSize;
            } else if (newSize <= m_mesh.m_nMaxSize) {
                if (newSize > idx) {
                    memset(&m_mesh.m_pData[idx], 0, (newSize - idx) * sizeof(FxPoint));
                }
                m_mesh.m_nSize = newSize;
            } else {
                i32 grow = m_mesh.m_nGrowBy;
                if (grow == 0) {
                    grow = idx / 8;
                    if (grow < 4) {
                        grow = 4;
                ...
                i32 newMax = m_mesh.m_nMaxSize + grow;
                if (newSize > newMax) {
                    newMax = newSize;
                }
                FxPoint* nd = (FxPoint*)RezAlloc(newMax * sizeof(FxPoint));
                memcpy(nd, m_mesh.m_pData, m_mesh.m_nSize * sizeof(FxPoint));
                memset(&nd[m_mesh.m_nSize], 0, (newSize - m_mesh.m_nSize) * sizeof(FxPoint));
                RezFree(m_mesh.m_pData);
                m_mesh.m_pData = nd;
                m_mesh.m_nSize = newSize;
                m_mesh.m_nMaxSize = newMax;
            }
            m_mesh.m_pData[idx] = pt;
        }
    }
    return 1;
}
