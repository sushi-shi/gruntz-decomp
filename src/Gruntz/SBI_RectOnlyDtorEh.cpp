...
// Stamp ??_7CSBI_RectOnly, run Reset, then MSVC folds the base dtor (stamp
// ??_7CStatusBarItem + Reset). The non-trivial base subobject supplies the
// /GX frame and the 0/-1 trylevel stamps.
//
// Reset() is the class's member teardown @0xe8760 (a 1-byte `ret` - CSBI_RectOnly adds
...
// so cl emits its inline body (`mov [ecx],??_7CStatusBarItem; jmp Reset`) as an
...
// ??_7CStatusBarItem/Reset by shape) and binds BOTH the funclet reloc @0x100700
// and this body's Reset @0x10bfa0.
