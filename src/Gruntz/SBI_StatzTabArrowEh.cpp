#include <rva.h>
#include <Ints.h>
// SBI_StatzTabArrowEh.cpp - the /GX EH-framed CSBI_StatzTabArrow destructor (C:\Proj\Gruntz). The split
// off the frameless base TU is matching-neutral (each function is RVA-keyed).
//
// REAL polymorphic hierarchy:  CSBI_StatzTabArrow : CSBI_ImageSetAni : CSBI_ImageSet : CSBI_Image : CSBI_RectOnly : CStatusBarItem.
// Each base subobject has a non-trivial (inline) virtual dtor, so MSVC folds the
// base teardown chain into the most-derived ~CSBI_StatzTabArrow and emits the full /GX SEH
...
struct CSBI_ImageSetAni : CSBI_ImageSet {
    ...
    virtual void Ianf1();
    virtual void Ianf2();
    void DtorImageSetAni(); // reloc-masked member teardown
};
inline CSBI_ImageSetAni::~CSBI_ImageSetAni() { DtorImageSetAni(); }
