...
// The held DirectDraw palette interface (DDRAW COM, __stdcall). SetEntries is slot 6
// (+0x18): QueryInterface/AddRef/Release/GetCaps/GetEntries/Initialize precede it.
// Never constructed here (only the interface ptr is received) - minimal view; the
// full interface is IDirectDrawPaletteZ in <Gruntz/CDirectDrawMgr.h>.
struct IDirectDrawPaletteZ {
    struct Vtbl {
        void* m_slots0_14[6]; // +0x00..+0x14 (QI/AddRef/Release/GetCaps/GetEntries/Initialize)
        void(__stdcall* SetEntries)(
            IDirectDrawPaletteZ* self,
            int flags,
            int start,
            int count,
            void* entries
...
void CDDScreen::UploadPalette() {
    ...
    m_palette->m_vtbl->SetEntries(m_palette, 0, 0, 0x100, m_slots);
}
