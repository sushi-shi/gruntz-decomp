...
// src/Stub/Discovered.cpp (trace-attributed to CPlay: this->m_c at +0xc is the
// CView, m_c->m_8 the renderer that owns the plane list, m_c->m_24 the draw
...
// plane descriptor's type field (m_7c->m_10, a reloc-masked fn-ptr compare).
//
...
// The plane descriptor at plane->m_7c: +0x10 is the type discriminator (a
...
struct Plane {
    char pad0[4];
    i32 m_4; // +0x04
    i32 m_8; // +0x08 flags (|= 0x10000 when consumed)
    char pad0c[0x5c - 0xc];
    i32 m_5c;  // +0x5c  x
    i32 m_60;  // +0x60  y
    Vec4 m_64; // +0x64  by-value block C
    char pad74[0x7c - 0x74];
    PlaneDesc* m_7c; // +0x7c descriptor
    ...
    i32 m_130;  // +0x130
    Vec4 m_134; // +0x134 block F
    Vec4 m_144; // +0x144 block E
    Vec4 m_154; // +0x154 block D
    i32 m_164;  // +0x164
    i32 m_168;  // +0x168
};
...
// The grid geometry at drawSurf->m_5c.
struct GridGeom {
    char pad0[0x20];
    i32* m_20; // +0x20 cell table
    i32* m_24; // +0x24 row table
    char pad28[0x30 - 0x28];
    i32 m_30; // +0x30 tile count X
    i32 m_34; // +0x34 tile count Y
    char pad38[0x8c - 0x38];
    i32 m_8c; // +0x8c shift X
    i32 m_90; // +0x90 shift Y
};

// The draw surface at m_c->m_24.
struct DrawSurf {
    char pad0[0x4c];
    TileObj** m_4c; // +0x4c object table
    char pad50[0x5c - 0x50];
    GridGeom* m_5c; // +0x5c grid geometry
};

// The renderer at m_c->m_8: an embedded plane list at +0x10.
struct Renderer {
    char pad0[0x10];
...
struct CView {
    char pad0[0x8];
    Renderer* m_8; // +0x08 renderer (owns the plane list)
    char padc[0x24 - 0xc];
    DrawSurf* m_24; // +0x24 draw surface
};

// The per-level sinks the scans feed. m_2e4 receives the rebuilt tile/plane
// records (AddToList1 / the big covered-tile draw); m_2dc receives the extra
...
// The two plane-type discriminators the scans compare m_7c->m_10 against
...
public:
    ...
    ObjSink2dc* m_2dc; // +0x2dc
    char pad2e0[0x2e4 - 0x2e0];
    ObjSink2e4* m_2e4; // +0x2e4
...
// rebuild its record onto m_2e4->AddToList1, for each covered-tile plane sample
...
i32 CPlay::ScanBuildTiles() {
    PlaneList* pl = (PlaneList*)((char*)m_c->m_8 + 0x10);
    if (pl == 0) {
        return 0;
    }
    ...
    while (pos != 0) {
        ...
        if (p->m_134.a == (i32)0x80000000) {
            p->m_134.a = 0;
        }
        if (p->m_144.a == (i32)0x80000000) {
            p->m_144.a = 0;
        }
        if (p->m_154.a == (i32)0x80000000) {
            p->m_154.a = 0;
        }
        if (p->m_64.a == (i32)0x80000000) {
            p->m_64.a = 0;
        }
        void* vf = p->m_7c->m_10;
        if (vf == (void*)PlaneType_Rock) {
            ...
            if (m_2e4->AddToList1(
                ...
                == 0) {
                CString s;
                PlaneErrFmt(&s, "Bad rock at: x=%d, y=%d", p->m_5c, p->m_60);
                g_64556c->ReportError(s);
                return 0;
            }
            if (p->m_11c == 0x32) {
                m_2dc->InsertPtr(p->m_118, p->m_114);
            }
            p->m_8 |= 0x10000;
        } else if (vf == (void*)PlaneType_Covered) {
            DrawSurf* ds = m_c->m_24;
            i32 x = p->m_5c;
            i32 y = p->m_60;
            if (x < 0) {
                x = 0;
            } else {
                i32 lim = ds->m_5c->m_30;
                if (x >= lim) {
                    x = lim - 1;
                }
            }
            ...
            i32 cell = g->m_20[g->m_24[tileY] + tileX];
            i32 tile;
            if (cell == (i32)0xeeeeeeee || cell == (i32)0xffffffff) {
                tile = 0;
            } else {
                tile = ds->m_4c[cell & 0xffff]->Query(subX, subY);
            }
            if (m_2e4->BigDraw(
                    tile,
                    0x1a,
                ...
                == 0) {
                CString s;
                PlaneErrFmt(&s, "Bad covered powerup at: x=%d, y=%d", p->m_5c, p->m_60);
                g_64556c->ReportError(s);
                return 0;
            }
            if (p->m_11c == 0x32) {
...
// list applying the permutation - remapping m_124 for the five ordinary quad
// types and scatter-permuting the four m_134 corners for the special type.
// ---------------------------------------------------------------------------
// @early-stop
// CByteArray Fisher-Yates shuffle-idiom wall (~77.7%, topic:codegen-idiom): the
// guard + the whole plane-walk (5 quad types -> perm[m_124]; the special type's
...
i32 CPlay::ScanShuffleQuads() {
    ...
    while (pos != 0) {
        ...
        if (vf == (void*)PlaneQuadA || vf == (void*)PlaneQuadB || vf == (void*)PlaneQuadC
            ...
            i32 scatter[4];
            scatter[perm[0]] = p->m_134.a;
            scatter[perm[1]] = p->m_134.b;
            scatter[perm[2]] = p->m_134.c;
            scatter[perm[3]] = p->m_134.d;
            p->m_134.a = scatter[0];
            p->m_134.b = scatter[1];
            p->m_134.c = scatter[2];
            p->m_134.d = scatter[3];
