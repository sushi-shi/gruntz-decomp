...
// (0x1614b0 the buffer-release - re-homed to src/Gruntz/GameLevel.cpp as
// CImageSet3::FreePixels (vtable slot 6, ~??_7CImageSet3@@6B@+0x18): the polymorphic
// CImageSet3 there already declared the slot-6 slot; the B_1614b0 view dissolved onto
// it, m_14 == CImageSet3::m_pixels.)
...
// (0x184fb0 Fwd_184fb0 - the cursor-position forwarder - re-homed to
// src/Gruntz/DebugPrintf.cpp, which is its real home: the RezDebugPrintf*XY variants
// there call it to position the cursor (the earlier "no caller" trace missed those
// debug-printf call sites). Not dead code.)
...
// (0x13dee0 B_13dee0::Set - the frame-timing setter - re-homed to src/Rez/RezMgr.cpp
// (its RVA neighborhood, next to SpinWaitUntil). Its sibling TrySet below stays here
// and calls Set out-of-line; the B_13dee0 view remains in BoundaryUpperViews.h.)
...
// (0x13ee30 B_13ee30::WaitFlip - the surface flip-wait loop - re-homed to
// src/Image/Image.cpp (its RVA neighborhood, next to CDDSurface::Clear), with its
// IDDS_ee30/B_13ee30 views; dissolved from BoundaryUpperViews.h.)
...
// (0x151d20 B_151d20::Notify - the hooked-callback notify - re-homed to
// src/Wwd/WwdGameObject.cpp (its RVA neighborhood); the B_151d20/Cb151d20 views stay in
// BoundaryUpperViews.h, which that TU already includes.)
