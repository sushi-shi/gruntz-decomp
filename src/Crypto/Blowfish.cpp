...
DATA(0x0021aeb0)
static u32 s_bf_P[bf_N + 2] = BF_PI_P_INIT;
DATA(0x0021aef8)
static u32 s_bf_S[4][256] = BF_PI_S_INIT;
DATA(0x0021bef8)
static u32 s_bf_P_Orig[bf_N + 2] = BF_PI_P_INIT;
DATA(0x0021bf40)
static u32 s_bf_S_Orig[4][256] = BF_PI_S_INIT;

union aword {
    u32 m_word;
    u8 m_byte[4];
    struct {
        unsigned int m_byte3 : 8;
        unsigned int m_byte2 : 8;
        unsigned int m_byte1 : 8;
        unsigned int m_byte0 : 8;
    } m_w;
};

#define S(x, i) (s_bf_S[i][x.m_w.m_byte##i])
#define bf_F(x) (((S(x, 0) + S(x, 1)) ^ S(x, 2)) + S(x, 3))
#define ROUND(a, b, n) (a.m_word ^= bf_F(b) ^ s_bf_P[n])
...
i16 InitializeBlowfish(const char* key, i16 keybytes) {
    ...
    memcpy(s_bf_P, s_bf_P_Orig, (bf_N + 2) * sizeof(u32));
    memcpy(s_bf_S, s_bf_S_Orig, 4 * 256 * sizeof(u32));

    j = 0;
    for (i = 0; i < bf_N + 2; ++i) {
        temp.m_word = 0;
        temp.m_w.m_byte0 = static_cast<u8>(key[j]);
        temp.m_w.m_byte1 = static_cast<u8>(key[(j + 1) % keybytes]);
        temp.m_w.m_byte2 = static_cast<u8>(key[(j + 2) % keybytes]);
        temp.m_w.m_byte3 = static_cast<u8>(key[(j + 3) % keybytes]);
        data = temp.m_word;
        s_bf_P[i] = s_bf_P[i] ^ data;
        j = (j + 4) % keybytes;
    }
