...
i32 CSaveGame::InitializeSaveDirectory(const char* saveDirectory) {
    ...
    memset(m_header, 0, s_SAVE_FILE_HEADER_BYTES);
    Init();
    Load();
...
i32 CSaveGame::Load() {
    ...
    file.Read(m_header, s_SAVE_FILE_HEADER_BYTES);
...
i32 CSaveGame::Save(char* screenshotPath, i32 messageId) {
    ...
    ComputeAll();
    file.Write(m_header, s_SAVE_FILE_HEADER_BYTES);
...
i32 CSaveGame::CheckMagic() {
    i32 v = m_magic;
    return v == s_SAVE_PROGRESS_MAGIC;
}

RVA(0x000e56b0, 0x8)
void CSaveGame::SetMagic() {
    m_magic = s_SAVE_PROGRESS_MAGIC;
}
