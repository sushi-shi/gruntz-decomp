// NetMgrGame.cpp - the Gruntz-side half of the (still-conflated) CNetMgr: the
// multiplayer game-state / protocol cluster at 0x0b5xxx-0x0bdxxx.
//
// SPLIT NOTE (2026-07-10): what was one god-TU (src/Net/NetMgr.cpp, unit netmgr)
// actually holds TWO separate retail objects (a ~0xbaf30 .text gap between them):
//   * the real engine CNetMgr - the DirectPlay wrapper at 0x178xxx (retail
//     C:\Proj\NetMgr\NetMgr.cpp) - now split out to src/Net/NetMgr.cpp;
//   * THIS file - the game-side multiplayer session/protocol (SetupMultiplayer
//     session, SetupServices, PollSession, the stat-send family, channel table +
//     chat, player-drop/version handling, config load/save). NetMgr.h documents
//     these as really CMulti (RTTI CMulti:CPlay:CState); they are kept labeled
//     CNetMgr:: for now (the conflated class stays; only the TU is split), the
//     same way netmgrmenuselect/netmgrwait/netcmdsession already split off other
//     CNetMgr methods into their own units.
//
// Handlers here: OnMultiOptions/OnMultiPause/OnOutOfSync fire a multiplayer
// command through the engine dispatcher; HandleControlMsg is the net control-
// message switch; OnPlayerLeft/RecordDropPlayer2 tear down leaving players.
