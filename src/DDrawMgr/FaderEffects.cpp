...
i32 CFaderRadial::ApplyInit(CFaderConfig* desc) {
    ...
    SET_VECTOR2_COMPONENTS(
        m_center,
        static_cast<i32>(s->m_apiDesc.dwWidth) / 2,
        static_cast<i32>(s->m_apiDesc.dwHeight) / 2
    );
    ...
    for (i32 y = 0; y < static_cast<i32>(m_srcSurface->m_apiDesc.dwHeight); y++) {
        for (i32 x = 0; x < static_cast<i32>(m_srcSurface->m_apiDesc.dwWidth); x++) {
            ...
            SET_VECTOR2_COMPONENTS(
                cell.m_velocity,
                static_cast<float>(dx) * fade,
                static_cast<float>(m_center.m_y - y) * fade
            );
...
void CFaderRadial::RenderFrame(i32 frame) {
    ...
    for (i32 i = 0; i < static_cast<i32>(m_srcSurface->m_apiDesc.dwWidth)
        ...
        if (d > g_faderOne) {
            ...
            Coord pixel(
                m_center.m_x + static_cast<i32>((m_cells[i].m_velocity.m_x / sf)),
                m_center.m_y - static_cast<i32>((m_cells[i].m_velocity.m_y / sf))
            );
            if (pixel.m_x > 0 && pixel.m_x < m_dstSurface->m_apiDesc.dwWidth && pixel.m_y > 0
                && pixel.m_y < m_dstSurface->m_apiDesc.dwHeight) {
                base[pixel.m_y * m_dstSurface->m_apiDesc.lPitch + pixel.m_x] = m_cells[i].m_pixel;
            }
        }
    }
...
i32 CFaderLight::ApplyInit(CFaderConfig* desc) {
    ...
    CPoint pt(m_center.m_x, m_center.m_y);
    ...
    if (m_clearMode != false) {
        ...
        if (m_height > 0) {
            do {
                m_spanStarts[i] = m_center.m_x;
                m_spanEnds[i] = m_center.m_x;
                i++;
            } while (i < m_height);
        }
    }
...
void CFaderLight::RenderFrame(i32 frame) {
    ...
    if (m_clearMode != false) {
        ...
        for (;;) {
            ...
            if (row >= m_center.m_y - r + 1 && row <= r + m_center.m_y - 1) {
                i32 right;
                i32 left;
        ...
        for (;;) {
            i32 stop = delta + frame + m_spanCount + m_center.m_y;
            if (stop >= m_height) {
                stop = m_height;
            }
            if (row >= stop) {
            ...
            if (row > m_center.m_y - frame && row < frame + m_center.m_y) {
                i32 right;
                i32 left;
...
i32 CFaderLight::GetFrameCount() {
    ...
    double pBottom = pow(
        static_cast<double>(static_cast<i32>(m_targetSurface->m_apiDesc.dwHeight) - m_center.m_y),
        g_faderPowK
    );
    double pRight = pow(
        static_cast<double>(static_cast<i32>(m_targetSurface->m_apiDesc.dwWidth) - m_center.m_x),
        g_faderPowK
    );
