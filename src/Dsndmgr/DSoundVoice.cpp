...
//
// A voice drives a single cloned DirectSound buffer over a volume ramp: from
// m_rampStartVolume -> m_rampEndVolume across m_rampDurationMs starting at
// m_rampStartTime, with m_stopRewindOnFinish gating final StopAndRewind. The two
// methods modeled here are vtable slots 0 (Tick, 0x137060) and 1 (Stop,
// 0x1370d0); both call the buffer's IsPlaying / SetVolumeByIndex /
// StopAndRewind wrappers (all reloc-masked __thiscall externs).
//
...
struct DSoundVoice {
    ...
    i32 m_stopRewindOnFinish;  // +0x14  stop+rewind-when-finished flag
    i32 m_rampEndVolume;       // +0x18  ramp end volume index
    i32 m_rampStartVolume;     // +0x1c  ramp start volume index
    i32 m_rampDurationMs;      // +0x20  ramp duration (ms)
    i32 m_rampStartTime;       // +0x24  ramp start timestamp
};
...
i32 DSoundVoice::Tick_137060(i32 now) {
    ...
    if (done && m_stopRewindOnFinish != 0) {
