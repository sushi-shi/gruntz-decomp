...
struct CSurfacePalette {
    char m_pad0[0x2c];
    IPalSink* m_paletteSink; // +0x2c  palette sink interface
    char m_pad30[0x108 - 0x30];
    u8 m_colorTable[0x400]; // +0x108  256 * 4-byte color slots
...
void CSurfacePalette::ResetPalette() {
    for (i32 i = 0; i < 256; i++) {
        m_colorTable[i * 4 + 0] = 0;
        m_colorTable[i * 4 + 1] = 0;
        m_colorTable[i * 4 + 2] = 0;
    }
    m_paletteSink->m_vptr->SetEntries(m_paletteSink, 0, 0, 0x100, m_colorTable);
}
