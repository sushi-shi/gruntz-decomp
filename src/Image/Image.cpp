// Image.cpp - the engine's DirectDraw surface class (DIRSURF.CPP leaf methods).
//
// CDDSurface IS the DirectDraw surface object (the full surface layout + COM thunks:
// Lock/BltEx/SetColorKey, reloc-masked). It is the polymorphic BASE of the shared
// pool-item surface family (9-slot vtable @0x5ef7f0). This TU holds the DIRSURF
// surface ops: the format resolvers (Resolve/ResolveEx), the file-format DECODERS
// (DecodeBmp/DecodePcx/DecodePid + the low-level DecodePcxData), the file consumers
// (LoadBmp/LoadPcx/LoadPid/DecodePcxEx), the blit dispatchers (Blit/Blit248/BlitDirect),
// the save path (SaveFile/SaveDispatch/SaveRle16), the surface teardown (~CDDSurface/
// FreeSurfaces), the CImageFactory factory (Build_13e9a0), and the derived
// CFileImageSurface wrapper's dtor/scalar-delete. Retail RVA band 0x13e070-0x148ac0.
//
// The REZ image loaders (CRezImage / CImageExtLoader, RVA band 0x1757c0-0x1772e0) were
// split out to src/Image/RezImage.cpp so each class de-fragments into its own TU.
//
// CDDSurface / CFileImageSurface / CRezSurfaceItem are all real-polymorphic (cl emits
// their ??_7 and stamps the vptr in the ctor/dtor; the shared surface vtable 0x5ef7f0
// reloc-masks). The CFileIO stack objects carry a dtor -> a C++ EH frame -> /GX.
#include <Image/Image.h>
#include <Rez/RezMgr.h> // RezAlloc/RezFree - the engine allocator the decoders use
#include <rva.h>
// <string.h>: strcpy ("BM" file header) / memset / memcpy in the decoders + save path.
#include <string.h>
// CDDSurface is the DirectDraw surface object; the surface virtuals it dispatches
// (BlitSurf slot 3, IsValid slot 5, v20 slot 8) are genuine virtual calls on `this`.
// CDirectDrawMgr is included only for CDirectDrawMgr::GetErrorString (the Fill error path).
#include <DDrawMgr/DirectDrawMgr.h>
#include <DDrawMgr/DDrawPtrCollections.h> // the palette context the decoders read (m_palBpp/m_palette/m_hasPalette)
#include <ddraw.h> // real IDirectDrawSurface dispatch (this->m_8->Unlock/Blt/Release)
#include <Globals.h>
...
// ===========================================================================
// CDDSurface surface helpers (DIRSURF.CPP leaf methods). The rich surface layout +
// COM thunks are CDDSurface's own (it IS the DirectDraw surface object), reached
// directly via members/methods - no separate wrapper-class view. Placed in retail-RVA
// order (all below the CDDSurface decoders' RVAs, so they lead the CDDSurface section).
// ===========================================================================
...
// with the RTTI CImage cluster in CImage.h. CDDSurface is the shared DIRSURF surface,
// now single-sourced in this header (the former include/Image/CDDSurface.h was folded
