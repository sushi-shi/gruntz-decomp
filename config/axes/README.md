...
| flipvertical.json | CRezImage::FlipVertical 0x176840 | 6 (22 over three rounds) | 47.36 -> **61.76**. Two independent wins, both about what cl is allowed to strength-reduce. (a) `m_height` must stay a MEMBER read inside the loop: hoisting it to a local lets cl turn the bottom-row offset into a `-wid` accumulator, which eliminates `i` outright and flips the outer latch to `dec/jne` where retail has `cmp/jl` (+5.6). (b) the bottom offset must be a local formed BETWEEN loop 1 and loop 2, shared by loops 2 and 3, exactly where retail computes it (+4.8); forming it before loop 1 costs 5.2, and `botRow * wid` inside the two loops instead of a pre-multiplied `botOff` costs 11. NEGATIVE on row POINTERS in every form - `&m_pixels[..]` scores 45.0 and hoisted walked pointers (`top += wid; bot -= wid`) crater to 14.1. The manifest's committed cell is the product optimum |
