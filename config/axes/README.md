...
| kitchenslime-clamp.json | CKitchenSlime::CKitchenSlime 0xb23a0 | 64 | SUPERSEDED 2026-08-01 - the matrix only enumerated TERNARY spellings, which all tie because cl5 always loads the condition's LEFT operand first. The if-WIDEN STATEMENT form (`i32 hi = far; if (cand > hi) hi = cand;`) loads the far edge first into the result register and fixed all three clamps byte-exactly: 96.97 -> 97.60, branch sequences now AGREE. See docs/patterns/minmax-clamp-if-widen-not-ternary.md |
| resolvearrivalreposition.json | CGrunt::ResolveArrivalReposition 0xec670 | 6 | 91.31 -> **95.21** and the SIZE lands on retail's 664 B: both spans first, THEN both origins, with the origin doubling as the rolled point. Interleaving span/origin per axis enregisters outY and collapses the frame from `sub esp,0xc` to one dword |
| resolvearrivalreposition-origin.json | CGrunt::ResolveArrivalReposition 0xec670 | 5 | NEGATIVE - the spans-then-origins order above is the optimum; swapping the two origins (91.14), the two spans (90.78) or re-interleaving (91.31) all lose. The last 4.8% is the outX/outY frame-SLOT pair (retail +0x14/+0x10, ours +0x10/+0x14) plus retail's third, unused slot |
| setuptcpipconfig.json | CMulti::SetupTcpIpConfig 0xbc460 | 12 | NEGATIVE - the size already matches (590 B); the residual is three instructions of scheduling in the RegisterChannelFrom block and no declaration/call spelling moves it (96.00-96.10). Inlining `ch0->m_008` into the arg list craters to 85.45 (drops 48 B and 4 relocs) |
