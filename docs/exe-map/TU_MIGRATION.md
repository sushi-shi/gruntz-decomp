...
| `0x184610-0x185a0e` | 50 | ~~seam-glued~~ **RESOLVED waveM** (dossier #16c): MenuPage-obj COMDAT cluster `0x184610-0x1848a6` + FIVE pocket objs (rezcoll / debugprintf / rezlist / zlib-uncompr / wapuncompress) + MenuItem obj `0x185460+` | 0.02 | boundaries 0x1848b0 / 0x185460 confirmed; tree layout already correct |
...
| `0x03ac30-0x03e135` | 39 | ~~WOVEN~~ **RESOLVED waveM** (dossier #16a): TWO objs - CustomWorldDialog.cpp `[0x3ac10..0x3bc78]` / Demo.cpp `[0x3bfa0..0x3dee1]` + the GruntStartingPoint actreg cluster `0x3df30+` | 0.30 | dissolved: customworldinfodlg+customlevellist+customwwdpath+wwdfile-statics->customworlddialog; demosetup+worldlevelkey+democameratools+animworkerhandlers+butemgrparse+orphanleaves-strays->demo; actreg4->gruntstartingpoint |
...
| `0x0e5ad0-0x0e8733` | 22 | ~~mixed~~ **RESOLVED waveM** (dossier #16b): PER-CLASS objs (one-file-per-class SBI items, each headed by a 9-frag static group) - aniplayer-leaf / wellgoo / image / imageset / imagesetani / menuitem / thin-rectonly (NEW SBI_RectOnlyBase.cpp) | 0.24 | the weave was attribution noise; vtable-slot proof re-attributed 12 bodies; CAniPlayer folded onto the CSBI_ImageSetAni chain |
...
| sbi_rectonly | ~~3 intervals~~ `0xe5ad0` RESOLVED waveM (thin-chain fns -> SBI_ImageSet.cpp / SBI_RectOnlyBase.cpp); `0xfdc00` (23) + `0x104d60` (58) = the 0x570 HOST, open |
...
| orphanleaves | ~~2 intervals~~ RESOLVED waveM: `0x3ac30` members re-homed (customworlddialog / demo / gruntstartingpoint); `0x183d0` core kept |
...
- `0x03ac30-0x03e135` (19 EH sites): **RESOLVED waveM** - both objs /GX (customworlddialog eh, demo eh; prologues at 0x3b470/0x3b940/0x3bb50 + 0x3c0e0/0x3cc20)
...
- `0x184610-0x185a0e` (3 EH sites): **RESOLVED waveM** - the EH sites are the MenuPage-obj COMDAT dtors (menuitem stays eh to host them); pocket objs base (zero sites)
