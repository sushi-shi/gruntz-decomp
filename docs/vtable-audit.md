...
the retail `.rdata` (VA = RVA + 0x400000), `config/retail/data_vtables.tsv` (the realized
...
| :--------------- | :------- | :-------------- | :--- |
| `g_zDArrayVtbl` | 0x1e70fc (0x5e70fc) | `??_7?$zDArray@P8CUserLogic@@AEHXZ@@6B@` | templated — can't `VTBL()`, stays `data_vtables.tsv` |
...
`MidiSequence`, `CRezArchive`, `CRezItmBase`, the CDDraw worker family, etc.
...
`new`s + stamps; object class in another TU; not in `data_vtables.tsv`):
