...
> `config/library_labels.csv` row (a carved LIBRARY body, excluded from the match
...
> - **data** — `DATA(0x..)` / `DATA_SYMBOL(..)` (a named game global or `??_7` vtable).
>
...
>
> The overlap gate deliberately checks LOW rows too. Once source evidence claims
> such an RVA, the stale diagnostic row must be pruned rather than left to contradict
> that evidence.
>
