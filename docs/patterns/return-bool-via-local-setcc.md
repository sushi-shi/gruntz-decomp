# Storing a call result in a named local before comparing it emits the `setcc`-in-scratch form, not `neg/sbb`
tags: cpp:return cpp:local cpp:branch | asm:sete asm:setne asm:test | topic:codegen-idiom
symptoms: retail returns a 0/1 bool via `xor edx,edx; test eax,eax; setcc dl; mov eax,edx` (a scratch-register setcc) but your recompile emits the branchless `neg eax; sbb eax,eax; inc/neg eax` arithmetic form
confidence: 8/10
...
The result type must remain authentic. A pointer-returning call uses a pointer local and a NULL
comparison; naming the already-normalized integer is byte-neutral:

```cpp
CGiantRockLogic* found = ScanNeighborhood(tileX, tileY);
return found != NULL;      // xor scratch; test eax,eax; setne; mov eax,scratch
```
