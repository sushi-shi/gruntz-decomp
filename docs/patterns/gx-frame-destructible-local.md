# A destructible stack local (CString/CFileIO/CRegMgr) forces the `/GX` frame → `flags="eh"`
...
`CFileIO`, a `CRegMgr`) MSVC5 wraps the body in a `/GX` exception-handling frame
...
```toml
[[unit]]  # flags = "eh"  when a CString/CFileIO/CRegMgr local is in scope
```
