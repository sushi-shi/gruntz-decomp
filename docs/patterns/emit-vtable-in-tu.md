...
build/gen/bindings.tsv). cl then emits the vtable in-TU and the vptr store binds locally. Getting the
...
build/gen/bindings.tsv to the `U` form; bodies are byte-identical virtual-vs-nonvirtual so greens held);
