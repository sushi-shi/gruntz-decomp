...
moved ddrawsubmgr -> ddrawsurfacemgr, so the `RVA_COMPGEN` pin moves with it; the
function census (`config/retail/gruntz_functions.tsv`) keys on RVAs, so the
transfer passes and the row re-homes on the next full build. If retail's
placement for that COMDAT sits inside the OLD unit's span,
